...
export function ShopFloorMap({ machines, onSelectMachine }: ShopFloorMapProps) {
 ...
 const oeePct = m.oee != null ? (m.oee <= 1 ? Math.round(m.oee * 100) : Math.round(m.oee)) : null;
 ...
 <span
  ...
  oeePct == null ? 'text-muted' : oeePct >= 80 ? 'text-success-fg' : oeePct >= 65 ? 'text-warning-fg' : 'text-danger-fg'
  }`}
 >
 {oeePct == null ? '—' : `${oeePct}%`}
 </span>
 ...
 <div
  ...
  oeePct == null ? 'bg-elevated' : oeePct >= 80 ? 'bg-success-bg' : oeePct >= 65 ? 'bg-warning-bg' : 'bg-danger-bg'
  }`}
 ...
 <p className="text-2xs text-muted">Click any machine cell to inspect the latest OEE, mold, and work-order data.</p>
 </div>
 <div className="flex items-center gap-3 text-xs">
  <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-success-bg" /> Running</span>
  <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-warning-bg" /> Maintenance</span>
  <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-danger-bg" /> Breakdown</span>
 ...
 <span className="font-mono font-medium text-primary">{selectedMachine.active_mold ?? '—'}</span>
 </div>
 <div>
 <span className="text-muted block text-2xs uppercase">Active Work Order</span>
 <span className="font-mono font-medium text-accent">{selectedMachine.active_wo ?? '—'}</span>
 </div>
 <div>
 <span className="text-muted block text-2xs uppercase">Cycle Time</span>
 <span className="font-mono font-medium text-primary">{selectedMachine.cycle_time_sec == null ? '—' : `${selectedMachine.cycle_time_sec}s`}</span>
 </div>
 <div>
