  ...
  z
    ...
    .refine(
      (v) => {
        ...
        return typeof len === 'number' ? d === len : d >= len[0] && d <= len[1];
      },
      {
        message:
          typeof len === 'number'
...
export const employeeSchema = z
  .object({
    first_name: z
      ...
      .min(1, 'Required')
      .max(100)
      .regex(namePattern, "Letters, spaces, ., ', - only"),
    middle_name: z
      .string()
      .max(100)
      .regex(namePattern, "Letters, spaces, ., ', - only")
    ...
    mobile_number: z
      ...
      .refine((v) => digitsOnly(v).length === 11 && digitsOnly(v).startsWith('09'), {
    ...
    emergency_contact_phone: z
      ...
      .refine((v) => digitsOnly(v).length >= 7 && digitsOnly(v).length <= 15, {
    ...
    basic_monthly_salary: z
      .string()
      .optional()
      .refine((v) => !v || moneyPattern.test(v), { message: 'Numbers only, up to 2 decimals' }),
    semi_monthly_rate: z
      .string()
      .optional()
      .refine((v) => !v || moneyPattern.test(v), { message: 'Numbers only, up to 2 decimals' }),
    ...
    bank_account_no: z
      .string()
      .max(50)
      .regex(/^[A-Za-z0-9\-\s]*$/, 'Letters, digits, spaces, hyphens only')
      .optional()
      .or(z.literal('')),
  ...
  .refine(
    (d) =>
      ...
      (!!d.basic_monthly_salary &&
        moneyPattern.test(d.basic_monthly_salary) &&
        Number(d.basic_monthly_salary) > 0),
    { message: 'Enter a valid monthly salary greater than 0', path: ['basic_monthly_salary'] },
  )
  .refine(
    (d) =>
      d.__edit === true ||
      d.pay_type !== 'semi_monthly' ||
      (!!d.semi_monthly_rate &&
        moneyPattern.test(d.semi_monthly_rate) &&
        Number(d.semi_monthly_rate) > 0),
    { message: 'Enter a valid semi-monthly rate greater than 0', path: ['semi_monthly_rate'] },
  )
  .refine((d) => !d.basic_monthly_salary || Number(d.basic_monthly_salary) <= 9_999_999.99, {
    message: 'Maximum 9,999,999.99',
    path: ['basic_monthly_salary'],
  })
  .refine((d) => !d.semi_monthly_rate || Number(d.semi_monthly_rate) <= 9_999_999.99, {
    message: 'Maximum 9,999,999.99',
    path: ['semi_monthly_rate'],
  });
...
export function EmployeeForm({
  ...
  registerSetError,
  submitLabel,
  canEditSensitive: sensitivePermission = true,
}: Props) {
  ...
  const minBirthStr = localIsoDate(new Date(Date.now() - 1000 * 60 * 60 * 24 * 365.25 * 15));

  return (
    <form
      onSubmit={handleSubmit(onSubmit, onFormInvalid<EmployeeFormValues>(FIELD_LABELS))}
      className="max-w-4xl mx-auto px-5 py-4 space-y-8"
    >
      ...
      <Section title="Employment">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Input
            ...
            type="date"
            max={todayStr}
            {...register('date_regularized')}
            error={errors.date_regularized?.message}
          />
          ...
          {isEditing && (
            <div className="col-span-2 rounded-md border border-default bg-elevated/30 px-3 py-2 text-sm text-muted">
              ...
              <Link className="text-link hover:underline" to="/hr/employees">
                Open salary adjustments
              </Link>
              .
            </div>
          )}
      ...
      <Section
        ...
        hint="Optional. Assign a shift during creation. Can also be changed later from the employee profile."
      >
        <div className="grid grid-cols-2 gap-3">
      ...
      {canEditSensitive ? (
        <Section
          ...
          hint="Stored encrypted at rest. Digits-only — formatting is automatic."
        >
          <div className="grid grid-cols-2 gap-3">
            <Controller
              name="sss_no"
