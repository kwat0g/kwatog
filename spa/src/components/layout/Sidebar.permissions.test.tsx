...
describe('role-aligned sidebar permissions', () => {
  ...
  it('gives the driver a way to their own delivery run sheet', () => {
    ...
    const driver = {
      permissions: new Set(['supply_chain.driver.access']),
      features: allFeatures,
      roleSlug: 'driver',
    };
    const warehouse = {
      permissions: new Set(['supply_chain.deliveries.view']),
      features: allFeatures,
  ...
  it('exposes the merged Warehouse Map only to warehouse structure managers', () => {
    ...
    expect(
      SECTIONS.flatMap((s) => s.items).some((entry) => entry.to === '/inventory/stock-count'),
    ).toBe(false);
  });
