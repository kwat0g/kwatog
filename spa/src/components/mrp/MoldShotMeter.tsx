...
export function MoldShotMeter({
 ...
 <span>0 Shots</span>
 <span className="text-amber-600 font-medium">{warningRatioPct}% Threshold</span>
 ...
 <span className="font-mono font-medium text-primary tabular-nums">{formatInt(remainingShots)}</span>
 </div>
 <div className="bg-surface p-2 rounded-md border border-default/50">
 <span className="text-2xs text-muted block">Tooling Status</span>
 <span className="font-mono font-medium capitalize text-primary">{status ?? 'Active'}</span>
 </div>
 </div>

 {/* Action Prompt */}
 {(isNearing || isExceeded) && (
 <div className="flex items-center justify-between gap-2 p-2.5 rounded-md bg-amber-500/10 border border-amber-500/30 text-xs">
 <div className="flex items-center gap-2">
