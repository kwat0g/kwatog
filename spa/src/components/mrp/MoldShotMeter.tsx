...
export function MoldShotMeter({
  ...
  return (
    <div className="bg-canvas border border-border rounded-xl p-4 shadow-xs space-y-3">
      ...
      <div className="space-y-1.5">
        <div className="flex justify-between items-baseline text-xs">
          ...
          <span className="font-mono text-sm font-bold text-primary tabular-nums">
            {formatInt(currentShots)} <span className="text-xs font-normal text-muted">/ {formatInt(maxShots)} shots</span>
          </span>
        </div>

        <div className="h-3 bg-surface rounded-full overflow-hidden border border-border/60 p-0.5 relative">
          <div
        ...
        <div className="flex justify-between items-center text-2xs text-muted font-mono pt-0.5">
          <span>0 Shots</span>
          <span className="text-amber-600 font-semibold">{warningRatioPct}% Threshold</span>
      ...
      <div className="grid grid-cols-3 gap-2 text-xs pt-1">
        <div className="bg-surface p-2 rounded-lg border border-border/50">
          <span className="text-2xs text-muted block">Used Lifespan</span>
          <span className="font-mono font-bold text-primary">{pct.toFixed(1)}%</span>
        </div>
        <div className="bg-surface p-2 rounded-lg border border-border/50">
          <span className="text-2xs text-muted block">Shots Remaining</span>
          <span className="font-mono font-bold text-primary tabular-nums">{formatInt(remainingShots)}</span>
        </div>
        <div className="bg-surface p-2 rounded-lg border border-border/50">
          <span className="text-2xs text-muted block">Tooling Status</span>
          <span className="font-mono font-bold capitalize text-primary">{status ?? 'Active'}</span>
        </div>
      </div>

      {/* Action Prompt */}
      {(isNearing || isExceeded) && (
        <div className="flex items-center justify-between gap-2 p-2.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-xs">
          <div className="flex items-center gap-2">
