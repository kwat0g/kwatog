...
import type {
  ...
  TrendDirection,
  ForecastPoint,
} from '@/types/forecasting-dashboard';
...
export function ForecastPanel({
  ...
  const chartCeiling =
    maxValue <= 100 && data.historical.every((p) => p.value <= 100) ? 100 : maxValue;
              ...
              >
                <div
                  className={`w-full rounded-sm transition-[height] duration-300 ${
                    p.isForecast ? 'bg-accent/30 border-t-2 border-accent/60' : 'bg-accent/60'
                  }`}
