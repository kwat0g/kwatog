...
function formatValue(summary: DashboardWidgetSummary): string {
  ...
  if (summary.kind === 'decimal') return Number(summary.value).toLocaleString(undefined, { maximumFractionDigits: 2 });
...
export function LiveDashboardWidget({
  ...
  return (
    <Panel
      title={widget.name}
      actions={href ? <Link to={href} className="text-xs text-link hover:underline">Open →</Link> : undefined}
    >
      {loading ? (
