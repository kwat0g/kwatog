...
export function DemandForecastPanel({
  ...
  const byProduct = new Map<
    string,
    { product: NonNullable<(typeof forecasts)[0]['product']>; forecasts: typeof forecasts }
  >();
  ...
  return (
    <Panel title={title} meta={`${forecasts.length} entries · ${year}`}>
      <div className="space-y-3">
        {entries.map(({ product, forecasts: prods }) => {
          const totalQty = prods.reduce((s, f) => s + f.forecasted_quantity, 0);
          const withActual = prods.filter(
            (f) => f.actual_quantity != null && f.forecasted_quantity !== 0,
          );
          const avgVariance =
            withActual.length > 0
              ? (withActual.reduce(
                  (s, f) =>
                    s + (f.actual_quantity! - f.forecasted_quantity) / f.forecasted_quantity,
                  0,
                ) /
                  withActual.length) *
                100
              ...
              <div className="flex items-center justify-between mt-1.5 text-2xs text-muted">
                ...
                {avgVariance != null && (
                  <Chip variant={Math.abs(avgVariance) > 20 ? 'warning' : 'success'}>
                    {avgVariance > 0 ? '+' : ''}
                    {avgVariance.toFixed(1)}% avg variance
                  </Chip>
                )}
              </div>
            </div>
