...
/**
 * Dry-run of the bank file. `unbankable_*` describes employees who would be left
 * out because they have no bank account on file — the server refuses to generate
 * such a file, so this is the warning shown beforehand.
 */
export interface BankFilePreview {
  ...
  total: string;
  count: number;
  unbankable_count: number;
  unbankable_amount: string;
  unbankable_sample: Array<{ employee_no: string | null; name: string; net_pay: string }>;
}
