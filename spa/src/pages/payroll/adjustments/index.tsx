...
export default function PayrollAdjustmentsPage() {
  ...
  const [confirmApprove, setConfirmApprove] = useState<PayrollAdjustment | null>(null);
  ...
  const labels = new Map(
    [...(adjustmentOptions?.statuses ?? []), ...(adjustmentOptions?.types ?? [])].map((option) => [
      option.value,
      option.label,
    ]),
  );
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ id, remarks }: { id: string; remarks: string }) =>
      adjustmentsApi.reject(id, remarks),
    onSuccess: () => {
  ...
  const filterConfig: FilterConfig[] = [
    {
      ...
      label: 'Status',
      type: 'select',
      options: [{ value: '', label: 'All' }, ...(adjustmentOptions?.statuses ?? [])],
    },
    {
      key: 'type',
      label: 'Type',
      type: 'select',
      options: [{ value: '', label: 'All' }, ...(adjustmentOptions?.types ?? [])],
    },
  ];
  ...
  const columns: Column<PayrollAdjustment>[] = [
    ...
    },
    {
      ...
      header: 'Type',
      cell: (r) => (
        <Chip variant={r.type === 'underpayment' ? 'info' : 'warning'}>
          {labels.get(r.type) ?? r.type_label}
        </Chip>
      ),
    },
    {
      ...
      header: 'Amount',
      align: 'right',
      cell: (r) => <NumCell className="font-medium">{formatPeso(r.amount)}</NumCell>,
    },
    ...
    },
    {
      key: 'created_at',
      header: 'Submitted',
      cell: (r) => <NumCell>{formatDate(r.created_at)}</NumCell>,
    },
    {
      key: 'status',
      header: 'Status',
      cell: (r) => (
        <Chip variant={statusVariant(r.status)}>{labels.get(r.status) ?? r.status_label}</Chip>
      ),
    },
    {
      key: 'actions',
    ...
    <div>
      ...
      {data && data.data.length === 0 && (
        <EmptyState
          ...
          action={
            can('payroll.adjustments.create') ? (
              <Button variant="primary" onClick={() => navigate('/payroll/adjustments/create')}>
                Raise adjustment
              </Button>
            ) : undefined
          }
      ...
      >
        ...
        <ModalFooter>
          ...
          <Button
            variant="danger"
            onClick={() =>
              rejectTarget && rejectMutation.mutate({ id: rejectTarget.id, remarks: rejectRemarks })
            }
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (confirmApprove) approveMutation.mutate(confirmApprove.id);
        }}
          ...
          confirmApprove ? (
            <>
              <span className="font-mono tabular-nums text-primary">
                {formatPeso(confirmApprove.amount)}
              </span>{' '}
              {labels.get(confirmApprove.type)?.toLowerCase() ??
                confirmApprove.type_label.toLowerCase()}{' '}
              for{' '}
              <span className="text-primary">
                {confirmApprove.employee?.full_name ?? 'this employee'}
              </span>
              {confirmApprove.period?.label ? <> against {confirmApprove.period.label}</> : null}.
              Once approved it is applied to the employee&rsquo;s next payroll run.
            </>
          ) : undefined
        }
        variant="warning"
