...
export default function PayrollPeriodDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    'employees' | 'failures' | 'anomalies' | 'summary' | 'variance'
  >('employees');
  ...
  const {
    ...
    enabled: !!id,
    refetchInterval: (q) =>
      isProcessing(q.state.data as PayrollPeriod | undefined) ? 3000 : false,
  });
  ...
  useEcho<PayrollProgressEventPayload>(`payroll.period.${id}`, '.payroll.progress', (payload) => {
    qc.setQueryData(
      ['payroll-period', id],
      (prev: PayrollPeriod | undefined) =>
        prev && {
          ...prev,
  ...
  // Dry-run the bank file so a shortfall is visible BEFORE the user clicks
  // download. The server refuses to generate a file that would leave someone out
  // for want of bank details (that money would otherwise be recognised as paid
  // in the GL but never actually sent), so without this the only feedback is an
  // error toast after the attempt.
  const { data: bankPreview } = useQuery({
    queryKey: ['payroll-bank-file-preview', period?.id, bankFormat],
    queryFn: () => periodsApi.bankFilePreview(period!.id, bankFormat),
    // canBankFile is declared further down (it needs `period` to be loaded), so
    // the same condition is spelled out here rather than referenced before its
    // initialisation.
    enabled:
      Boolean(period?.id) &&
      Boolean(bankFormat) &&
  ...
  const finalizeMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Period finalized — GL posting queued.');
      qc.invalidateQueries({ queryKey: ['payroll-period', id] });
    },
    onError: (err: { response?: { data?: { message?: string } } }) =>
  ...
  const canCompute =
    can('payroll.periods.compute') && period.status === 'draft' && !period.is_thirteenth_month;
  const canRecompute =
    can('payroll.periods.compute') && period.status === 'computed' && !period.is_thirteenth_month;
  ...
  const columns: Column<Payroll>[] = [
    {
      ...
      cell: (r) =>
        r.employee ? (
          <StackedCell
            primary={<span>{r.employee.full_name}</span>}
    ...
    },
    {
      key: 'pay_type',
      header: 'Pay Type',
      cell: (r) => <span className="capitalize">{r.pay_type}</span>,
    },
    {
      key: 'days_worked',
      header: 'Days',
      align: 'right',
      cell: (r) => <NumCell>{r.days_worked ?? '—'}</NumCell>,
    },
    {
      key: 'gross_pay',
      header: 'Gross',
      align: 'right',
      cell: (r) => <NumCell>{formatPeso(r.gross_pay)}</NumCell>,
    },
    {
      key: 'total_deductions',
      header: 'Deductions',
      align: 'right',
      cell: (r) => <NumCell>{formatPeso(r.total_deductions)}</NumCell>,
    },
    {
      ...
      cell: (r) => (
        <NumCell>
          {Number(r.adjustment_amount) === 0 ? '—' : formatPeso(r.adjustment_amount)}
        </NumCell>
      ),
    },
    {
      ...
      header: 'Net',
      align: 'right',
      cell: (r) => <NumCell className="font-medium">{formatPeso(r.net_pay)}</NumCell>,
    },
    {
      key: 'status',
      header: 'Status',
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          <>
            Payroll date <span className="font-mono">{formatDate(period.payroll_date)}</span> ·
            created by {period.creator?.name ?? '—'}
        ...
        actions={
          <>
            ...
            {period.is_auto_created && (
              <span
                title={
                  period.auto_created_at
                    ? `Auto-scheduled ${period.auto_created_at}`
 ...
 running job looked indistinguishable from a hung one. */}
            ...
            {canBankFile && bankBlocked && (
              <div className="w-full mt-2 rounded border border-danger/40 bg-danger-bg/5 p-3">
                ...
                <ul className="mt-1.5 space-y-0.5">
                  {(bankPreview?.unbankable_sample ?? []).map((row) => (
                    <li key={row.employee_no ?? row.name} className="text-xs text-muted">
                      <span className="font-mono">{row.employee_no}</span> {row.name} —{' '}
                      <span className="font-mono">{formatPeso(row.net_pay)}</span>
                    </li>
                  ))}
                </ul>
                <p className="mt-1.5 text-xs text-muted">
                  Add their bank details in HR, then generate the file again.
                </p>
              </div>
            )}
            {canUploadProof && (
      ...
      <div className="px-5 py-4 grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5">
        ...
        <div>
            ...
            <div className="flex items-start gap-2 px-3 py-2 mb-4 bg-danger-bg text-danger-fg rounded-md text-xs">
              ...
              <div>
                ...
                {period.voided_at && (
                  <>
                    {' '}
                    on <span className="font-mono">{formatDate(period.voided_at)}</span>
                  </>
                )}
                {period.voider?.name && <> by {period.voider.name}</>}
                {period.gl_entry_number ? ' — its GL posting was reversed.' : '.'}
                {period.void_reason && (
                  <div className="mt-0.5 text-primary/80">Reason: {period.void_reason}</div>
                )}
              </div>
            </div>
          )}
          ...
          {activeTab === 'anomalies' && <AnomalyReviewPanel periodId={period.id} />}

          {activeTab === 'summary' && (
            <Panel title="Deduction summary">
              <div className="text-xs text-muted">
                Aggregated deduction breakdown is available per-employee in their detail view.
                Future enhancement: show period totals by deduction type here.
              </div>
            </Panel>
          )}
            ...
            {!period.disbursement_proofs || period.disbursement_proofs.length === 0 ? (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted">
                  No proof uploaded yet. After transferring salaries, upload the bank deposit slip
                  or transaction confirmation here.
                </p>
                {canUploadProof && (
                  <Button
                    variant="secondary"
    ...
    <Panel title="Period-over-period variance">
      <div className="space-y-4">
        <div className="max-w-sm">
          ...
          <Select value={compareToId} onChange={(e) => onCompareToChange(e.target.value)}>
        ...
        {compareToId && isLoading && <SkeletonTable columns={4} rows={4} />}
              ...
              <thead>
                <tr className={theadTrCls}>
                  ...
                  <Th align="right">
                    Previous
                    <br />
                    <span className="font-mono text-2xs normal-case">
                      {varianceData.previous.period_label}
                    </span>
                  </Th>
                  <Th align="right">
                    Current
                    <br />
                    <span className="font-mono text-2xs normal-case">
                      {varianceData.current.period_label}
                    </span>
                  </Th>
                ...
                {[
                  ...
                  <tr key={row.label} className={trCls}>
                    ...
                    <Td align="right" mono className={deltaColor(row.delta)}>
                      {row.delta > 0 ? '+' : ''}
                      {row.isMoney ? fmt(row.delta) : row.delta}
                    </Td>
...
function DisbursementProofCard({
  proof,
  periodId,
}: {
  proof: DisbursementProof;
  periodId: string;
}) {
  const qc = useQueryClient();
  ...
  const restoreMutation = useMutation({
    mutationFn: () => periodsApi.restoreProof(periodId, proof.id),
    onSuccess: () => {
    ...
    <div className="flex items-start gap-3 rounded-md border border-default bg-canvas p-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-0.5 text-xs text-muted">
          Uploaded by {proof.uploader?.name ?? '—'} ·{' '}
          {proof.created_at ? formatRelative(proof.created_at) : ''}
        </div>
      </div>
            ...
            <Button
              ...
              onClick={() =>
                void downloadAuthenticatedFile(periodsApi.downloadProof(periodId, proof.id), {
                  openInNewTab: true,
                  errorMessage: 'Failed to open disbursement proof.',
                })
              }
              className="text-muted hover:text-primary"
            />
    ...
    <Modal isOpen={open} onClose={onClose} size="md" title="Upload Disbursement Proof">
      <div className="space-y-3 py-3">
        <Select label="Proof type" value={proofType} onChange={(e) => setProofType(e.target.value)}>
          {(proofOptions?.proof_types ?? []).map((type) => (
      ...
      <ModalFooter>
        ...
        <Button
          ...
          disabled={!file || !!disbursedError || mutation.isPending}
