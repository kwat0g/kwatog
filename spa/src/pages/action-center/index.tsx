...
export default function ActionCenterPage() {
  ...
  // Triage actions from the former Exception Workbench. Task state is SHARED:
  // a snooze or resolve hides the item for every role, so Reopen and Unclaim
  // sit beside them — anything one role can hide, every role must be able to
  // undo.
  const triage = useMutation({
    mutationFn: ({ action, snoozed_until }: { action: 'claim' | 'unclaim' | 'acknowledge' | 'snooze' | 'resolve' | 'reopen'; snoozed_until?: string }) =>
  ...
  return (
    <div>
      <PageHeader
        title="Action Center"
        subtitle="One prioritized queue for quality, maintenance, and operational exceptions. Approvals live in the Approval Queue."
        ...
        <div className="p-5 space-y-4">
          ...
          <section className="rounded-md border border-default bg-canvas">
            ...
            <div className="rounded-md border border-default bg-canvas p-2 flex items-center gap-2 flex-wrap">
              ...
              <Button size="sm" variant="secondary" disabled={!selected.size} onClick={() => triage.mutate({ action: 'unclaim' })}>
                <LuUserMinus size={12} /> Unclaim
              </Button>
              ...
              <Button size="sm" variant="secondary" disabled={!selected.size} onClick={() => triage.mutate({ action: 'reopen' })}>
                <LuInbox size={12} /> Reopen
              </Button>
                ...
                {visibleItems.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className={cn(
                        'px-3 py-3 flex items-start gap-3 transition-colors group hover:bg-elevated/40',
                        focusRingInset,
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-2 flex-wrap">
                          ...
                          {item.task_state === 'snoozed' && item.snoozed_until && (
                            <Chip variant="neutral">snoozed until {formatRelative(item.snoozed_until)}</Chip>
                          )}
                        ...
                        <span className="flex items-center gap-x-3 gap-y-1 flex-wrap mt-1.5 text-2xs text-subtle">
                          ...
                          {item.updated_by && <span>Updated by {item.updated_by.name}</span>}
                          {item.created_at && (
