...
import type { PortalInvoiceSummary } from '@/types/b2b';
...
export default function CustomerInvoicesPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['portal', 'customer', 'invoices', filters],
    queryFn: () => customerPortalApi.listInvoices(filters),
  ...
  const columns: Column<PortalInvoiceSummary>[] = [
    {
      key: 'invoice_number',
      header: 'Invoice #',
      cell: (r) => <span className="font-mono font-medium text-accent">{r.invoice_number}</span>,
    },
    {
      key: 'date',
      header: 'Date',
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          data ? (
            <>{data.meta.total} billing documents issued by <CompanyName /></>
          ) : (
      ...
      {data && (
        <div className="px-5 py-4">
          <DataTable
            tableKey="portal-customer-invoices"
            columns={columns}
            data={data.data}
            meta={data.meta}
            onPageChange={(page) => setFilters((current) => ({ ...current, page }))}
            onPageSizeChange={(per_page) => setFilters((current) => ({ ...current, per_page, page: 1 }))}
            onRowClick={(r) => navigate(`/portal/customer/invoices/${r.id}`)}
            emptyState={
              <EmptyState
                icon="receipt"
                title="No invoices"
