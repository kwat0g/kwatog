...
export default function SupplierDeliverySchedulesPage() {
 ...
 return <div>
  <PageHeader
   ...
   backLabel="Portal"
   actions={<Button variant="primary" size="sm" icon={showForm ? <LuX size={14} /> : <LuPlus size={14} />} onClick={() => setShowForm((open) => !open)}>{showForm ? 'Cancel' : 'New schedule'}</Button>}
  />
  <div className="px-5 py-4 space-y-4 max-w-5xl">
   {showForm && <Panel title="New delivery schedule">
    <form onSubmit={submitForm} className="space-y-4">
     ...
     {selectedPo.isLoading && <SkeletonBlock className="h-10 rounded-md" />}
     {selectedPo.data && <div className="space-y-2">
      <div className="flex items-center justify-between"><span className="text-xs font-medium text-muted">PO line items</span><Button type="button" variant="ghost" size="sm" icon={<LuPlus size={12} />} onClick={() => setForm((current) => ({ ...current, lines: [...current.lines, emptyLine(selectedPo.data?.items[0]?.id ?? '')] }))}>Add line</Button></div>
      {form.lines.map((line, index) => <div key={`${line.purchase_order_item_id}-${index}`} className="grid gap-2 rounded-md border border-default bg-surface p-2 sm:grid-cols-[minmax(0,1fr)_6rem_minmax(8rem,12rem)_auto] sm:items-start">
       ...
       {form.lines.length > 1 && <Button type="button" variant="ghost" size="sm" iconOnly icon={<LuX size={14} />} onClick={() => setForm((current) => ({ ...current, lines: current.lines.filter((_, lineIndex) => lineIndex !== index) }))} aria-label="Remove line" className="justify-self-end text-muted hover:text-danger-fg sm:justify-self-auto" />}
      </div>)}
     </div>}
     <div className="flex justify-end gap-2 pt-2"><Button type="button" variant="secondary" size="sm" onClick={() => setShowForm(false)}>Cancel</Button><Button type="submit" variant="primary" size="sm" loading={submit.isPending}>Submit schedule</Button></div>
    </form>
   </Panel>}

   {schedules.isLoading && !schedules.data && <div className="space-y-3">{Array.from({ length: 3 }).map((_, index) => <SkeletonBlock key={index} className="h-20 rounded-md" />)}</div>}
   {schedules.isError && <EmptyState icon="alert-circle" title="Could not load delivery schedules" action={<Button variant="secondary" onClick={() => schedules.refetch()}>Try again</Button>} />}
   {!schedules.isLoading && !schedules.isError && schedulesData.length === 0 && <EmptyState icon="clipboard-list" title="No delivery schedules yet" description="Submit your first delivery schedule using the button above." />}
   {!schedules.isLoading && !schedules.isError && schedulesData.length > 0 && <div className="space-y-3">
    {schedulesData.map((schedule) => <Panel key={schedule.id} bodyClassName="p-4 space-y-2">
     <div className="flex items-center justify-between"><div className="flex items-center gap-3"><p className="text-sm font-medium">{schedule.month}</p><span className="text-2xs text-muted">{schedule.purchase_order?.po_number ?? ''}</span><Chip variant={chipVariantForStatus(schedule.status)}>{schedule.status_label ?? schedule.status}</Chip></div><p className="text-2xs text-muted">{formatDate(schedule.created_at)}</p></div>
     <PortalTable><table className={tableCls}><thead><tr className={theadTrCls}><Th>Product</Th><Th align="right">Qty</Th><Th>Notes</Th></tr></thead><tbody>{schedule.lines.map((line, index) => <tr key={index} className={trCls}><Td>{line.product_name}</Td><Td align="right" mono className="font-medium">{line.quantity}</Td><Td className="text-muted">{line.notes ?? '—'}</Td></tr>)}</tbody></table></PortalTable>
    </Panel>)}
   </div>}
   {schedules.data && <DataTablePagination meta={schedules.data.meta} perPage={filters.per_page} onPageChange={(page) => setFilters((current) => ({ ...current, page }))} onPageSizeChange={(per_page) => setFilters((current) => ({ ...current, per_page, page: 1 }))} />}
  </div>
 </div>;
}
