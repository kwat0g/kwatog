...
export default function InspectionSpecsListPage() {
  ...
  const [filters, setFilters] = useUrlFilters<InspectionSpecListParams>({
    page: 1,
    per_page: 25,
    trashed: 'with',
  });
  ...
  const columns: Column<InspectionSpec>[] = [
      ...
      cell: (r) =>
        r.product ? (
          <span>
            ...
            {(r.product.deleted_at || r.product.is_active === false) && (
              <Chip variant="neutral" className="ml-2">
                Unavailable
              </Chip>
            )}
          </span>
    ...
    },
    {
      ...
      header: 'Updated',
      align: 'right',
      cell: (r) => <NumCell>{formatDateIso(r.updated_at)}</NumCell>,
    },
    {
      key: 'actions',
      header: '',
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          data ? `${data.meta.total} ${data.meta.total === 1 ? 'spec' : 'specs'}` : undefined
        }
        actions={
          can('quality.specs.manage') ? (
            <Button
              ...
              size="sm"
              icon={<LuPlus size={14} />}
              onClick={() => navigate('/quality/inspection-specs/new')}
            >
              New spec
            </Button>
          ) : undefined
