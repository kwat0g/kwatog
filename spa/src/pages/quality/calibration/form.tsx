...
const schema = z
  ...
  .refine(
    (v) =>
      ...
      v.next_calibration_date >= v.last_calibration_date,
    {
...
export default function CalibrationFormPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (record) => {
      ...
      toast.success(
        isEdit ? 'Calibration instrument updated' : 'Calibration instrument registered',
      );
      navigate(`/quality/calibration/${record.id}/edit`);
    },
    onError: (error) =>
      applyServerValidationErrors(error, setError, 'Could not save the calibration instrument.'),
  });
  ...
  if (isEdit && detail.isLoading) {
    return (
      <div>
        <PageHeader
          title="Edit calibration instrument"
          backTo="/quality/calibration"
          backLabel="Calibration register"
        />
  ...
  if (isEdit && detail.isError) {
    return (
      <div>
        ...
        <div className="max-w-3xl mx-auto px-5 py-4">
          <QueryErrorState
            subject="this calibration instrument"
  ...
  return (
    <div>
      <PageHeader
        title={isEdit ? 'Edit calibration instrument' : 'New calibration instrument'}
        backTo="/quality/calibration"
        backLabel="Calibration register"
      />
      <FormDraftBanner safety={safety} />
      <form
        onSubmit={handleSubmit((values) => mutation.mutate(values), onFormInvalid<FormValues>())}
        ...
        <fieldset className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Input
              ...
              error={errors.last_calibration_date?.message}
            />
            <Input
              label="Next due"
              type="date"
