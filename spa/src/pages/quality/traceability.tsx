      ...
      <div className="px-5 pt-4">
        <form onSubmit={onSubmit} className="flex items-end gap-2 max-w-2xl">
          <div className="flex-1">
            <label
              htmlFor="traceability-term"
              className="mb-1 block text-xs text-muted font-medium"
            >
      ...
      <div className="col-span-1">
        <SectionCard icon={<LuFactory size={14} />} title="Production batch" highlight>
          <dl className="text-xs space-y-1.5">
            <Row
              label="Batch"
              value={<span className="font-mono">{wo.batch_number ?? '—'}</span>}
            />
            <Row
              label="Work order"
              value={
            ...
            <Row
              ...
              value={
                wo.product ? (
                  <span>
                    <span className="font-mono">{wo.product.part_number}</span> · {wo.product.name}
                  </span>
                ) : (
                  '—'
                )
            ...
            <Row
              label="Machine"
              value={wo.machine ? `${wo.machine.machine_code} · ${wo.machine.name}` : '—'}
            />
            <Row label="Mold" value={wo.mold ? `${wo.mold.mold_code} · ${wo.mold.name}` : '—'} />
            <Row
              label="Status"
              value={<Chip variant="neutral">{wo.status_label ?? wo.status}</Chip>}
            />
            <Row
              label="Good qty"
              value={<span className="font-mono tabular-nums">{wo.quantity_good}</span>}
            />
            <Row
              label="Rejected"
              value={<span className="font-mono tabular-nums">{wo.quantity_rejected}</span>}
            />
          </dl>
        </SectionCard>
      </div>
              ...
              {inspections.map((i) => (
                <li key={i.id}>
                  ...
                  <span className="text-muted ml-2">
                    {i.stage_label ?? i.stage} · {i.status_label ?? i.status}
                  </span>
                </li>
              ))}
            </ul>
              ...
              {lots.map((l) => (
                <li key={l.id} className="border-l-2 border-subtle pl-2">
                  ...
                  {l.delivery && (
                    <div className="text-muted">
                      <Link
                        to={`/supply-chain/deliveries/${l.delivery.id}`}
            ...
            <ul className="text-xs space-y-3">
              {woRows.map((row) => (
                <li key={row.work_order.id} className="border-l-2 border-subtle pl-2">
                  <div>
                    <Link
                      to={`/quality/traceability?term=${encodeURIComponent(row.work_order.batch_number ?? '')}`}
                      className="font-mono text-accent hover:underline"
                    >
      ...
      <div className="col-span-1">
        <SectionCard icon={<LuTag size={14} />} title="Shipment lot" highlight>
          <dl className="text-xs space-y-1.5">
            ...
            <Row
              label="Date"
              value={<span className="font-mono tabular-nums">{lot.lot_date ?? '—'}</span>}
            />
            <Row
              label="Quantity"
              value={<span className="font-mono tabular-nums">{lot.quantity}</span>}
            />
            <Row
              label="Product"
              value={
                lot.product ? (
                  <span>
                    <span className="font-mono">{lot.product.part_number ?? '—'}</span>
          ...
          {!delivery ? (
            ...
            <dl className="text-xs space-y-1.5">
              <Row
                ...
                value={
                  <Link
                    to={`/supply-chain/deliveries/${delivery.id}`}
                    className="font-mono text-accent hover:underline"
                  >
              ...
              <Row
                label="Status"
                value={<Chip variant="neutral">{delivery.status_label ?? delivery.status}</Chip>}
              />
              <Row label="Delivered" value={formatDateTime(delivery.delivered_at)} />
              <Row label="Confirmed" value={formatDateTime(delivery.confirmed_at)} />
            </dl>
          )}
        </SectionCard>
        ...
        <SectionCard icon={<LuBuilding2 size={14} />} title="Backward — supplier GRN">
          {!grn ? (
            ...
            <dl className="text-xs space-y-1.5">
              ...
              <Row
                label="Received"
                value={<span className="font-mono tabular-nums">{grn.received_date ?? '—'}</span>}
              />
            </dl>
          )}
        </SectionCard>
      ...
      <div className="col-span-1">
        <SectionCard icon={<LuPackage size={14} />} title="Material lot" highlight>
          <dl className="text-xs space-y-1.5">
            <Row
              label="Lot"
              value={<span className="font-mono">{ml.material_lot_number ?? '—'}</span>}
            />
            <Row
              label="Supplier ref"
              value={<span className="font-mono">{ml.supplier_lot_reference ?? '—'}</span>}
            />
            <Row
              label="Item"
              value={
                ml.item_code ? (
                  <span>
                    <span className="font-mono">{ml.item_code}</span>
                    {ml.item_name ? ` · ${ml.item_name}` : ''}
                  </span>
                ) : (
                  '—'
                )
            ...
            <Row
              label="Received"
              value={<span className="font-mono tabular-nums">{ml.quantity_received ?? '—'}</span>}
            />
            <Row
              label="Accepted"
              value={<span className="font-mono tabular-nums">{ml.quantity_accepted ?? '—'}</span>}
            />
          </dl>
        </SectionCard>
      </div>
