          ...
          {results.isLoading ? (
              ...
              <EmptyState
                ...
                description={
                  search
                    ...
                    : 'Or pick one of the stuck documents on the right.'
                }
              />
            </div>
          ) : (
            orders.map((so) => (
              <button
                ...
                className={cn(
                  'flex w-full items-center gap-3 py-2.5 text-left transition-colors hover:bg-subtle cursor-pointer',
                  focusRingInset,
                )}
              >
        ...
        >
          {bottlenecks.isLoading ? (
            ...
            <div className="py-4">
              <EmptyState
                icon="alert-circle"
                title="Couldn’t load bottlenecks"
                description="Try refreshing."
                action={
                  <LinkButton
                    onClick={() => bottlenecks.refetch()}
            ...
            <div className="py-4">
              <EmptyState
                icon="shield"
                title="Nothing stuck"
                description="Every chain is moving. Nice."
              />
            </div>
          ) : (
    ...
    return (
      <EmptyState
        ...
        action={
          <LinkButton
            onClick={onClear}
            icon={<LuArrowRight size={13} className="rotate-180" />}
            className="text-sm"
          >
            Back to search
          </LinkButton>
        ...
        <div className="space-y-4 lg:col-span-2">
          <Panel title="Order">
            <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3 text-sm">
              ...
              <dd className="col-span-2 font-mono tabular-nums font-medium text-primary">
                {peso(so.total_amount)}
              </dd>
            </dl>
            <div className="mt-4 border-t border-subtle pt-3">
              <Link
                to={`/crm/sales-orders/${so.id}`}
                        ...
                        items: [
                          {
                            ...
                            chip: {
                              variant:
                                so.mrp_plan.status === 'active'
                                  ? ('success' as const)
                                  : so.mrp_plan.status === 'cancelled'
                                    ? ('danger' as const)
                                    : ('neutral' as const),
                              text: so.mrp_plan.status_label ?? so.mrp_plan.status,
                            },
                          },
                        ],
                      },
              ...
              items={[
                {
                  ...
                  text: (
                    <>
                      Sales order <span className="font-mono">{so.so_number}</span> created.
                    </>
                  ),
                  time: formatDateIso(so.created_at, ''),
                },
                ...(so.status !== 'draft'
                  ? [
                      {
                        ...
                        text: (
                          <>
                            Status: <span className="font-medium">{so.status_label}</span>
                          </>
                        ),
                        time: formatDateIso(so.updated_at, ''),
                      },
                    ]
                  : []),
              ]}
