...
export default function DemandForecastingPage() {
  ...
  const methodLabels = new Map(
    (methodsQ.data?.methods ?? []).map((option) => [option.value, option.label]),
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (lookback === undefined && settingsQ.data)
      setLookback(settingsQ.data.default_lookback_months);
  ...
  const mrpInclusionM = useMutation({
    ...
    onSuccess: (product) => {
      toast.success(
        product.include_forecast_in_mrp
        ...
        {accuracyQ.data?.data?.mape !== null && accuracyQ.data?.data?.mape !== undefined && (
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            ...
            <StatCard
              label="Forecast Bias"
              value={
                accuracyQ.data.data.bias == null
                  ? '—'
                  : `${accuracyQ.data.data.bias > 0 ? '+' : ''}${accuracyQ.data.data.bias.toFixed(1)}%`
              }
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <Input
                  type="number"
                  min={settingsQ.data?.minimum_horizon_months}
                  max={settingsQ.data?.maximum_horizon_months}
                  value={horizon ?? ''}
                  disabled={!forecastingSettingsReady}
              ...
              <div>
                ...
                <Input
                  type="number"
                  min={settingsQ.data?.minimum_lookback_months}
                  max={settingsQ.data?.maximum_lookback_months}
                  value={lookback ?? ''}
                  disabled={!forecastingSettingsReady}
                ...
                label={
                  <>
                    ...
                    <span className="ml-2 text-xs text-muted">
                      Advisory only; no PR is created.
                    </span>
                  </>
                }
              />
          ...
          <div className="p-4">
            {!productId ? (
              <EmptyState
                ...
                description="Pick a product from the dropdown to view its demand history."
              />
              ...
              <EmptyState
                ...
                description="No confirmed sales orders match the selected scope."
              />
            ) : (
                      ...
                      <div className="relative w-full h-44 flex flex-col-reverse gap-0.5">
                        {d.actual > 0 && (
                          <div
                            ...
                            className="w-full bg-accent/70 rounded-t-sm transition-colors group-hover:bg-accent"
                        ...
                        {d.forecast > 0 && (
                          <div
                            ...
                            className="w-full bg-warning-bg/70 border border-warning rounded-t-sm transition-colors group-hover:bg-warning-bg"
          ...
          <div className="overflow-x-auto">
            <table className={tableCls}>
              ...
              <tbody>
                {!productId ? (
                  <tr>
                    <Td align="center" className="text-muted" colSpan={7}>
                      Select a product.
                    </Td>
                  </tr>
                  ...
                  <tr>
                    <Td align="center" className="text-muted" colSpan={7}>
                      No forecasts yet — click <em>Recompute</em>.
                    </Td>
                  </tr>
                ) : (
                  (forecastsQ.data ?? []).map((f) => (
                    <tr key={f.id} className={trCls}>
                      <Td>
                        {MONTH_NAMES[f.forecast_month - 1]} {f.forecast_year}
                      </Td>
                      <Td>
                        <Chip variant="neutral">
                          {f.method_label ??
                            methodLabels.get(f.method as 'moving_avg' | 'weighted_avg') ??
                            f.method}
                        </Chip>
                      </Td>
                      <Td align="right" mono>
                        {f.forecasted_quantity.toFixed(2)}
                      </Td>
                      <Td align="right" mono>
                      ...
                      <Td align="right" mono>
                        {canManage && (
                          <Button size="sm" variant="ghost" onClick={() => openManual(f)}>
                            Override
                          </Button>
                        )}
                      </Td>
