...
export default function MrpPlansListPage() {
  ...
  const statusLabels = new Map(
    (planOptions?.statuses ?? []).map((option) => [option.value, option.label]),
  );
  ...
  const columns: Column<MrpPlan>[] = [
    {
      key: 'no',
      header: 'Plan #',
      cell: (r) => <span className="font-mono">{r.mrp_plan_no}</span>,
    },
    {
      key: 'so',
      header: 'Sales order',
      cell: (r) =>
        r.sales_order ? <span className="font-mono">{r.sales_order.so_number}</span> : '—',
    },
    ...
    },
    {
      ...
      header: 'Auto PRs',
      align: 'right',
      cell: (r) => <NumCell>{r.auto_pr_count}</NumCell>,
    },
    {
      key: 'wo',
      header: 'Draft WOs',
      align: 'right',
      cell: (r) => <NumCell>{r.draft_wo_count}</NumCell>,
    },
    {
      key: 'status',
      header: 'Status',
    ...
    },
    {
      ...
      header: 'Generated',
      align: 'right',
      cell: (r) => <NumCell>{formatDateIso(r.generated_at, '')}</NumCell>,
    },
  ];
  ...
  const filterConfig: FilterConfig[] = [
    {
      ...
      label: 'Status',
      type: 'select',
      options: [{ value: '', label: 'All' }, ...(planOptions?.statuses ?? [])],
    },
  ];
  ...
  return (
    <div>
      <PageHeader
        title="MRP plans"
        subtitle={
          data ? `${data.meta.total} ${data.meta.total === 1 ? 'plan' : 'plans'}` : undefined
        }
        actions={
          can('mrp.runs.trigger') ? (
            <Button
              variant="primary"
              size="sm"
              icon={<LuPlay size={14} />}
      ...
      {isError && (
        <EmptyState
          icon="alert-circle"
          title="Failed to load MRP plans"
          action={
      ...
      {data && data.data.length > 0 && (
        <div className="px-5 py-4">
          <DataTable
            tableKey="mrp-plans"
            onRowClick={(r) => navigate(`/mrp/plans/${r.id}`)}
            columns={columns}
            data={data.data}
            meta={data.meta}
