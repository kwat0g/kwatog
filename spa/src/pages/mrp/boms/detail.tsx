  ...
  return (
    <div>
      <PageHeader
        title={
          <div className="flex items-center gap-3">
            ...
            <Chip variant={data.is_active ? 'success' : 'neutral'}>
              v{data.version}
        ...
        actions={
          can('mrp.boms.manage') ? (
            <div className="flex gap-2">
              ...
              >
                <LuRefreshCw
                  className={`h-3.5 w-3.5 mr-1 ${recalculate.isPending ? 'animate-spin' : ''}`}
                />{' '}
                Recalculate cost
              </Button>
              <Button
      ...
      <div className="px-5 py-4 space-y-4">
        <Panel
          title="Costing"
          meta={data.costed_at ? `Updated ${formatDateTime(data.costed_at)}` : 'Not calculated'}
        >
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="mt-1 text-sm">
                {data.cost_warnings?.length
                  ? `${data.cost_warnings.length} item${data.cost_warnings.length === 1 ? '' : 's'} at zero cost`
                  : 'None'}
              </div>
            </div>
          </div>
          {data.cost_warnings?.length > 0 && (
            <div className="mt-4 space-y-1 text-xs text-warning-fg">
              {data.cost_warnings.map((warning) => (
                <div key={`${warning.type}-${warning.item_code}`}>{warning.message}</div>
              ))}
            </div>
          )}
        </Panel>
        <Panel
          title="Materials"
          meta={`${data.item_count} ${data.item_count === 1 ? 'line' : 'lines'}`}
          noPadding
        >
            ...
            <tbody>
              {data.items?.map((m, i) => (
                <tr key={m.id} className={trCls}>
                  ...
                  <Td align="right" mono>
                    {Number(m.quantity_per_unit).toFixed(4)}
                  </Td>
                  <Td>{m.unit}</Td>
                  <Td align="right" mono>
                    {Number(m.waste_factor).toFixed(2)}
                  </Td>
                  <Td align="right" mono className="font-medium">
                    {Number(m.effective_quantity).toFixed(4)}
                  </Td>
                  <Td align="right" mono>
                    {m.cost_quantity === null ? '—' : Number(m.cost_quantity).toFixed(6)}
                  </Td>
                  <Td align="right" mono>
                    {m.unit_cost === null ? '—' : formatPeso(m.unit_cost)}
                  </Td>
                  <Td align="right" mono className="font-medium">
                    {m.extended_cost === null ? '—' : formatPeso(m.extended_cost)}
                  </Td>
                  <Td>
                    {m.cost_source === 'bom_rollup' ? (
                      <Chip variant="info">Subassembly roll-up</Chip>
                    ) : m.cost_source === 'standard_cost' ? (
                      <Chip variant="neutral">Standard cost</Chip>
                    ) : (
                      '—'
                    )}
                  </Td>
