...
export default function BomsListPage() {
  ...
  const [filters, setFilters] = useUrlFilters<BomListParams>({ page: 1, per_page: 25 });
  ...
  const columns: Column<Bom>[] = [
    ...
    },
    {
      ...
      header: 'Total cost',
      align: 'right',
      cell: (r) => <NumCell>{formatPeso(r.total_cost ?? r.material_cost)}</NumCell>,
    },
    {
      key: 'active',
      header: 'Status',
        ...
        actions={
          can('mrp.boms.manage') ? (
            <Button
              ...
              size="sm"
              icon={<LuPlus size={14} />}
              onClick={() => navigate('/mrp/boms/create')}
            >
              Add BOM
            </Button>
          ) : undefined
