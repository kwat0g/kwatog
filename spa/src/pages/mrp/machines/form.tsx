...
const schema = z.object({
 machine_code: z.string().regex(/^[A-Z0-9-]{2,20}$/, 'Use 2–20 uppercase letters, digits, or hyphens.'),
 name: z.string().min(1, 'Name is required').max(100),
 tonnage: z.string().regex(/^\d+$/, 'Whole number of tons, or leave blank').optional().or(z.literal('')),
 machine_type: z.string().max(50).optional().or(z.literal('')),
 operators_required: z.string().regex(/^\d+(\.\d)?$/, 'Use a number with up to 1 decimal').optional().or(z.literal('')),
 available_hours_per_day: z.string().regex(/^\d+(\.\d)?$/, 'Use 0–24 with up to 1 decimal').refine((v) => v === '' || Number(v) <= 24, 'Max 24 hours').optional().or(z.literal('')),
});
...
export function MachineForm({ initial, mode }: Props) {
 ...
 operators_required: initial?.operators_required != null ? String(initial.operators_required) : '',
 available_hours_per_day: initial?.available_hours_per_day != null ? String(initial.available_hours_per_day) : '',
 },
 });

 useEffect(() => {
 if (initial) {
 setValue('machine_code', initial.machine_code);
 setValue('name', initial.name);
 setValue('tonnage', initial.tonnage != null ? String(initial.tonnage) : '');
 setValue('machine_type', initial.machine_type ?? '');
 setValue('operators_required', initial.operators_required != null ? String(initial.operators_required) : '');
 setValue('available_hours_per_day', initial.available_hours_per_day != null ? String(initial.available_hours_per_day) : '');
 }
 }, [initial, setValue]);
 ...
 <fieldset className="mb-8">
 <legend className="text-xs uppercase tracking-wider text-muted font-medium mb-4">Specifications</legend>
 <div className="grid grid-cols-2 gap-3">
 <Input
 label="Tonnage (T)"
 {...register('tonnage')}
 ...
 {isSubmitting || mutation.isPending ? 'Saving…' : mode === 'create' ? 'Create machine' : 'Save changes'}
 </Button>
 </div>
 </form>
 );
