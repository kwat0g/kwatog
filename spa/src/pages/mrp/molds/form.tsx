...
const intField = (max: number, msg: string) =>
 z.string().regex(/^\d+$/, msg).refine((v) => Number(v) >= 1 && Number(v) <= max, `Use 1–${max}`);

const schema = z.object({
 mold_code: z.string().regex(/^[A-Z0-9-]{2,20}$/, 'Use 2–20 uppercase letters, digits, or hyphens.'),
 ...
 setup_time_minutes: z.string().regex(/^\d+$/, 'Whole minutes, or leave blank').optional().or(z.literal('')),
...
export function MoldForm({ initial, mode }: Props) {
 ...
 setup_time_minutes: initial?.setup_time_minutes != null ? String(initial.setup_time_minutes) : '',
 ...
 useEffect(() => {
 if (initial) {
 setValue('mold_code', initial.mold_code);
 setValue('name', initial.name);
 setValue('product_id', initial.product?.id ?? '');
 setValue('cavity_count', String(initial.cavity_count));
 setValue('cycle_time_seconds', String(initial.cycle_time_seconds));
 setValue('output_rate_per_hour', String(initial.output_rate_per_hour));
 setValue('setup_time_minutes', initial.setup_time_minutes != null ? String(initial.setup_time_minutes) : '');
 setValue('max_shots_before_maintenance', String(initial.max_shots_before_maintenance));
 setValue('lifetime_max_shots', String(initial.lifetime_max_shots));
 ...
 <fieldset className="mb-8">
 <legend className="text-xs uppercase tracking-wider text-muted font-medium mb-4">Performance</legend>
 <div className="grid grid-cols-2 gap-3">
 <Input
 label="Cavity Count"
 required
 ...
 <fieldset className="mb-8">
 <legend className="text-xs uppercase tracking-wider text-muted font-medium mb-4">Maintenance</legend>
 <div className="grid grid-cols-2 gap-3">
 <Input
 ...
 {isSubmitting || mutation.isPending ? 'Saving…' : mode === 'create' ? 'Create mold' : 'Save changes'}
 </Button>
 </div>
 </form>
 );
