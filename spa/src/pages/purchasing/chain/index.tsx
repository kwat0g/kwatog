...
export default function ProcurementChainPage() {
  ...
  if (isError)
    return (
      <div>
        ...
        <EmptyState
          icon="alert-circle"
          title="Failed to load chain data"
          action={
            <LinkButton className="text-sm" onClick={() => refetch()}>
              Retry
            </LinkButton>
          }
        />
  ...
  const overdueUrgency =
    bl.bills_overdue > 0 ? 'warning' : bl.bills_unpaid > 0 ? 'info' : 'success';
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-mono tabular-nums font-medium">
                  {formatPeso(bl.bills_this_month)}
                </span>
              </div>
...
function FlowStep({ icon: Icon, label, count, active }: FlowStepProps) {
  return (
    <div
      className={`flex flex-col items-center gap-1 flex-1 px-2 py-2 rounded ${active ? 'bg-elevated' : 'opacity-50'}`}
    >
