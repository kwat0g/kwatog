...
export default function PurchaseRequestsListPage() {
  ...
  const statusLabels = new Map(
    (requestOptions?.statuses ?? []).map((option) => [option.value, option.label]),
  );
  ...
  const columns: Column<PurchaseRequest>[] = [
        ...
        <div>
          ...
          {r.is_urgent && (
            <Chip variant="danger" className="ml-1">
              <LuZap size={10} className="inline mr-0.5" />
              URGENT
            </Chip>
          )}
        </div>
    ...
    },
    {
      key: 'needed_by',
      header: 'Needed by',
      cell: (r) => <span className="font-mono">{formatDate(r.required_delivery_date)}</span>,
    },
      ...
      cell: (r) => (
        <span className="flex items-center gap-1.5">
          ...
          {r.has_overdue_approval && (
            <span
              title={`Approval pending beyond ${requestOptions?.approval_sla_hours ?? 'configured'} hours`}
            >
    ...
    },
    {
      ...
      header: 'Estimated',
      align: 'right',
      cell: (r) => <NumCell>{formatPeso(r.total_estimated_amount)}</NumCell>,
    },
  ...
  const filterConfig: FilterConfig[] = [
    {
      ...
      label: 'Status',
      type: 'select',
      options: [{ value: '', label: 'All' }, ...(requestOptions?.statuses ?? [])],
    },
    {
      key: 'priority',
      label: 'Priority',
      ...
      options: [
        { value: '', label: 'All' },
        ...(requestOptions?.priorities ?? []).map((priority) => ({
        ...
        actions={
          can('purchasing.pr.create') ? (
            <Button
              ...
              size="xs"
              icon={<LuPlus size={14} />}
              onClick={() => navigate('/purchasing/purchase-requests/create')}
            >
              New PR
            </Button>
          ) : null
