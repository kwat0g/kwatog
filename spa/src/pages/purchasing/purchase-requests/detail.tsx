...
export default function PurchaseRequestDetailPage() {
  ...
  const submit = useOptimisticStatusAction({
    ...
    nextStatus: 'pending',
    successMsg: 'Submitted for approval.',
    errorMsg: 'Failed to submit.',
    afterSuccess: () => setConfirm(null),
  });
  const saveSourcingMethod = useMutation({
    mutationFn: () =>
      purchaseRequestsApi.setSourcingMethod(id, sourcingMethod as PurchaseRequestSourcingMethod),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: detailKey });
      qc.invalidateQueries({ queryKey: ['purchasing', 'purchase-requests'] });
      toast.success('Sourcing method saved.');
    },
  ...
  const approve = useOptimisticStatusAction({
    ...
    nextStatus: 'approved',
    successMsg: 'Purchase request approved.',
  ...
  const reject = useOptimisticStatusAction<string>({
    ...
    afterSuccess: () => setRejectOpen(false),
  });
  const cancel = useOptimisticStatusAction({
    detailKey,
    mutationFn: () => purchaseRequestsApi.cancel(id),
    nextStatus: 'cancelled',
    successMsg: 'Purchase request cancelled.',
    errorMsg: 'Failed to cancel.',
    afterSuccess: () => setConfirm(null),
  });
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            {data.is_urgent && (
              <Chip variant="danger">
                <LuZap size={12} className="inline mr-0.5" />
                URGENT
              </Chip>
            )}
            ...
            {data.status === 'pending' &&
              (data.actions?.can_approve || data.actions?.can_reject) && (
                <>
                  {data.actions?.can_reject && (
                    <Button
                      size="xs"
                      variant="secondary"
            ...
            {data.actions?.can_print && (
              <Button
                ...
                onClick={() =>
                  void downloadAuthenticatedFile(purchaseRequestsApi.pdfUrl(data.id), {
                    openInNewTab: true,
                    errorMessage: 'Failed to generate purchase request PDF.',
                  })
                }
              >
                PDF
          ...
          <Panel title="Choose sourcing method">
            <p className="text-sm text-muted mb-3">
              {data.is_auto_generated
                ? 'Choose whether this system-generated request (reorder point, MRP or sales order shortage) should create a direct PO or enter competitive RFQ sourcing.'
                : 'Internal purchase requests use the Direct PO path.'}
            </p>
            <div className="grid sm:grid-cols-2 gap-3">
              <label className="flex gap-3 rounded-md border border-default p-3 cursor-pointer">
                <input
                  type="radio"
                  name="sourcing_method"
                  value="direct_po"
                  checked={sourcingMethod === 'direct_po'}
                  onChange={() => setSourcingMethod('direct_po')}
                />
                <span>
                  <strong className="block">Direct PO</strong>
                  <span className="text-xs text-muted">
                    Create draft PO automatically after final PR approval when supplier and price
                    data are complete.
                  </span>
                </span>
              </label>
              {data.is_auto_generated && (
                <label className="flex gap-3 rounded-md border border-default p-3 cursor-pointer">
                  <input
                    ...
                    name="sourcing_method"
                    value="rfq"
                    checked={sourcingMethod === 'rfq'}
                    onChange={() => setSourcingMethod('rfq')}
                  />
                  <span>
                    <strong className="block">Competitive RFQ</strong>
                    <span className="text-xs text-muted">
                      Hold the approved request for a sealed supplier bidding event.
                    </span>
                  </span>
                </label>
              )}
            ...
            <Button
              className="mt-3"
              variant="primary"
              disabled={!sourcingMethod}
              onClick={() => saveSourcingMethod.mutate()}
              loading={saveSourcingMethod.isPending}
            >
              Save sourcing method
            </Button>
          </Panel>
        )}
            ...
            <div className="flex-1">
              ...
              <div className="text-muted">
                {data.po_conversion_note ??
                  'Automatic conversion could not complete. Review the request and convert it manually.'}
              </div>
            </div>
        ...
        {data.status === 'approved' && data.po_conversion_status === 'partial' && (
          <div className="flex items-center gap-3 rounded-md border border-warning/40 bg-warning-bg/10 px-4 py-3 text-sm">
            ...
            <div className="flex-1">
              ...
              <div className="text-muted">
                {data.po_conversion_note ??
                  'Some lines were converted to purchase orders; the rest still need a supplier.'}
              </div>
            </div>
            {data.actions?.can_convert && (
        ...
        {data.status === 'approved' && data.po_conversion_status === 'pending' && (
          <div className="flex items-center gap-3 rounded-md border border-info/40 bg-info-bg/10 px-4 py-3 text-sm">
            ...
            <div>
              ...
              <div className="text-muted">
                The approved request is queued for purchase-order creation.
              </div>
            </div>
          </div>
        )}
        {data.status === 'converted' &&
          data.purchase_orders?.some((po) => po.is_auto_generated) && (
              ...
              <div>
                ...
                <div className="text-muted">
                  ...
                  {data.purchase_orders
                    .filter((po) => po.is_auto_generated)
                    .map((po) => (
                      <Link
                        key={po.id}
          ...
          <div className="flex items-center gap-3 rounded-md border border-success/40 bg-success-bg/10 px-4 py-3 text-sm">
            ...
            <div className="flex-1">
              ...
              <div className="text-muted">
                ...
                {data.purchase_orders
                  .filter((po) => po.bill?.status === 'draft')
                  .map((po) => (
                    <span key={po.id} className="inline-flex items-center gap-1.5">
                      ...
                      <span className="font-mono tabular-nums">
                        {formatPeso(po.bill!.total_amount)}
                      </span>
                      ...
                      {can('accounting.bills.create') && (
                        <Button
                          ...
                          size="xs"
                          icon={<LuSend size={11} />}
                          onClick={() => setPostBillId(po.bill!.id)}
                        >
                          Post
                        </Button>
                      )}
            ...
            <ChainHeader
              steps={buildP2pChain({
                ...
                bills: data.purchase_orders.flatMap((po) =>
                  po.bill
                    ? [{ id: po.bill.id, bill_number: po.bill.bill_number, status: po.bill.status }]
                    : [],
                ),
              })}
            />
        ...
        {data.budget_warning_level && (
          <div className="flex items-center justify-between gap-4 rounded-md border border-warning/40 bg-warning-bg/10 px-4 py-3 text-sm">
            ...
            {['exhausted', 'overdrawn'].includes(data.budget_warning_level) &&
              ...
              data.actions?.can_acknowledge_budget && (
                <Button
                  size="sm"
                  variant="secondary"
  ...
  raw lowercase role slugs), so it was removed on 2026-09-10. */}
        ...
        <div className="col-span-2 space-y-4">
          <Panel title="Header">
            <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <dt className="text-2xs uppercase tracking-wider text-muted">Priority</dt>
                <dd className="flex items-center gap-1">
                  {data.priority_label ?? data.priority}
                  {data.is_urgent && (
                    <span title={data.urgency_reason ?? ''}>
                      <LuTriangleAlert size={12} className="text-danger-fg" />
                    </span>
                  )}
                </dd>
              </div>
              <div>
                <dt className="text-2xs uppercase tracking-wider text-muted">Needed by</dt>
                <dd className="font-mono">{formatDate(data.required_delivery_date)}</dd>
              </div>
              <div>
                <dt className="text-2xs uppercase tracking-wider text-muted">Department</dt>
                <dd>{data.department?.name ?? '—'}</dd>
              </div>
              <div>
                <dt className="text-2xs uppercase tracking-wider text-muted">Template</dt>
              ...
              <div>
                ...
                <dd className="font-mono tabular-nums">
                  {formatPeso(data.total_estimated_amount)}
                </dd>
              </div>
              {data.reason && (
                ...
                <tbody>
                  {data.items?.map((l) => (
                    <tr key={l.id} className={trCls}>
                      ...
                      <Td>{l.unit}</Td>
                      <Td align="right" mono>
                        {formatPeso(l.estimated_unit_price)}
                      </Td>
                      <Td align="right" mono className="font-medium">
                        {formatPeso(l.estimated_total)}
                        {l.suggested_vendor && (
                          <div className="text-2xs text-muted mt-0.5">
                            Vendor: {l.suggested_vendor.name}
                          </div>
                        )}
                      </Td>
                    </tr>
                ...
                groups={[
                  {
                    ...
                    items: data.purchase_orders.map((po) => ({
                      ...
                      chip: {
                        variant: (po.status === 'received' || po.status === 'closed'
                          ? 'success'
                          : po.status === 'cancelled'
                            ? 'danger'
      ...
      <ConfirmDialog
        ...
        description={
          <>Once submitted, the PR enters the approval workflow and edits are no longer allowed.</>
        }
        confirmLabel="Submit"
        variant="primary"
      ...
      <ConfirmDialog
        ...
        description={
          <>
            Approving advances the workflow. The action is recorded against your account in the
            audit log.
          </>
        }
      ...
      <ConvertPrToPoModal
        purchaseRequest={convertOpen ? data : null}
