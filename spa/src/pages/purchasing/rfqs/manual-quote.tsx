...
export default function ManualRfqQuotePage() {
    ...
    () =>
      fromCentavos(
        Object.values(values).reduce(
          (total, value) =>
            value.status === 'quoted'
              ? total + Math.round((Number(value.quantity) || 0) * (Number(value.price) || 0) * 100)
              : total,
          0,
        ),
      ),
    ...
    () =>
      fromCentavos(
        ...
        toCentavos(itemsTotal) +
          toCentavos(treatment === 'exclusive' ? (vatAmount ?? '0.00') : '0.00') +
          toCentavos(freight) +
          toCentavos(other),
      ),
  ...
  const upload = useMutation({
    ...
    onSuccess: (document) => {
      ...
      toast.success('Quotation PDF uploaded privately.');
    },
  ...
  const capture = useMutation({
    ...
    onSuccess: () => {
      toast.success('Manual supplier quotation captured and submitted.');
      navigate(`/purchasing/rfqs/${id}`);
    },
    onError: () =>
      toast.error(
        'The manual quotation could not be captured. Check each line and the uploaded PDF.',
      ),
  });
  ...
  return (
    <div>
      <PageHeader
        title="Capture supplier quotation"
        subtitle={rfq.rfq_number}
        backTo={`/purchasing/rfqs/${id}`}
        backLabel="RFQ detail"
      />
      <div className="px-5 py-4 max-w-4xl space-y-4">
        <Panel title="Supplier and original quotation">
          <div className="grid sm:grid-cols-2 gap-3">
            <label className="block text-sm">
              Invited supplier
              <select
                className="mt-1 w-full h-10 border border-default rounded-md bg-canvas px-3"
                value={vendorId}
                onChange={(event) => {
              ...
              >
                ...
                {rfq.invitations?.map((invitation) => (
                  <option key={invitation.vendor?.id} value={invitation.vendor?.id}>
          ...
          <div className="flex items-center gap-3 mt-3">
            <Button
              ...
              disabled={!vendorId || !file}
              loading={upload.isPending}
            >
              Upload private PDF
            </Button>
            {documentId && (
              <span className="text-sm text-success-fg">PDF linked to this capture.</span>
            )}
          </div>
        </Panel>
        <Panel title="Prices and VAT">
          <p className="text-sm text-muted mb-3">
            Record how the supplier's paper quotation states VAT. The VAT amount is derived from the
            lines below — mirror the PDF, do not estimate.
          </p>
          ...
          <p className="text-xs text-muted mt-1">
            {treatment === 'exclusive'
              ? `VAT at ${vatPercent ?? 'the prevailing rate'} applies on top of the quoted amounts.`
              : treatment === 'inclusive'
                ? 'The quoted amounts already include VAT; the VAT component is extracted from them.'
                : 'Supplier is not VAT-registered or the sale is VAT-exempt.'}
          </p>
          <div className="grid sm:grid-cols-2 gap-3 mt-3">
            <Input
              ...
              helper={
                treatment === 'inclusive'
                  ? `Extracted from gross prices at ${vatPercent ?? 'the prevailing rate'} (12/112 rule).`
                  : treatment === 'none'
                    ? 'Declared without VAT, matching the paper quotation.'
                    : `Computed at ${vatPercent ?? 'the prevailing rate'} on the quoted lines and charges.`
              }
            />
            <Input
              ...
              helper="Delivery charge stated on the quotation. 0.00 if none."
            />
            <Input
              label="Other charges"
              inputMode="decimal"
        ...
        <Panel title="Structured response">
          <p className="text-sm text-muted mb-3">
            Captured responses are immutable submitted quote versions and remain sealed until the
            RFQ closes.
          </p>
          <div className="space-y-3">
            {rfq.items?.map((item) => {
              ...
              return (
                <div key={item.id} className="border border-default rounded-md p-3">
        ...
        <Panel title="Quotation summary">
          <dl className="space-y-1 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <dd className="font-mono tabular-nums">{formatPeso(vatAmount)}</dd>
            </div>
            <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          <Button variant="secondary" onClick={() => navigate(`/purchasing/rfqs/${id}`)}>
            Cancel
          </Button>
          <Button
            variant="primary"
            disabled={!vendorId || !documentId || capture.isPending}
