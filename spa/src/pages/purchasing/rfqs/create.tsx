...
export default function CreateRfqPage() {
  ...
  return <div>
    ...
    <div className="px-5 py-4 space-y-4 max-w-4xl">
      <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted">{['Requirements', 'Suppliers', 'Terms and documents', 'Review and publish'].map((label, index) => <button key={label} className={`px-2 py-1 rounded ${step === index + 1 ? 'bg-accent text-accent-fg' : 'bg-subtle'}`} onClick={() => setStep(index + 1)}>{index + 1}. {label}</button>)}</div>
      {step === 1 && <Panel title="Requirements"><div className="space-y-3"><Input label="RFQ title" value={title} onChange={(e) => setTitle(e.target.value)} /><Input label="Instructions" value={instructions} onChange={(e) => setInstructions(e.target.value)} /><div className="rounded border border-default p-3"><div className="text-2xs uppercase tracking-wider text-muted mb-2">PR snapshot</div>{pr.data.items?.map((item) => <div key={item.id} className="flex justify-between border-b border-subtle py-2 last:border-0"><span>{item.description}</span><span className="font-mono">{item.quantity} {item.unit ?? ''}</span></div>)}</div></div></Panel>}
