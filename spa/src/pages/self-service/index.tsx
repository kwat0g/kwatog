...
export default function SelfServiceHomePage() {
 ...
 {[1, 2, 3].map((i) => <SkeletonBlock key={i} className="h-[92px] rounded-md" />)}
 </div>
 ...
 shift ? `${shift.name} shift · ${String(shift.time_in).slice(0, 5)}–${String(shift.time_out).slice(0, 5)}` : null,
 ...
 <PageHeader
 title={title}
 subtitle={subtitleParts.join(' · ') || 'Your payslips, attendance, leave, and requests in one place'}
 actions={
...
function QuickActionsPanel({ payslipSubtitle }: { payslipSubtitle: string }) {
 ...
 ].filter((a): a is { to: string; Icon: typeof Receipt; title: string; subtitle: string } => Boolean(a));
 ...
 <ArrowRight
 size={14}
 className="text-text-subtle shrink-0 mt-1 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-fast"
 aria-hidden="true"
 />
 </Link>
 </li>
