...
const columns: Column<LeaveRequest>[] = [
  ...
  },
  {
    ...
    header: 'Status',
    cell: (r) => (
      <Chip variant={STATUS_CHIP[r.status] ?? 'neutral'}>{r.status_label ?? r.status ?? '—'}</Chip>
    ),
  },
];
  ...
  const file = useMutation({
    mutationFn: (v: FormValues) =>
      selfServiceApi.fileLeaveSelf({
        ...
        end_date: v.end_date,
        half_day_period:
          v.half_day_period === 'none' ? undefined : (v.half_day_period as 'am' | 'pm'),
          ...
          <form onSubmit={handleSubmit((v) => file.mutate(v))} className="space-y-4 py-4">
              ...
              <div className="rounded-md border border-default bg-surface px-3 py-2 text-xs">
                ...
                <div className="h-1.5 rounded-full bg-subtle overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${
                        Number(selectedBalance.total_credits) > 0
                          ? Math.min(
                              100,
                              (Number(selectedBalance.remaining) /
                                Number(selectedBalance.total_credits)) *
                                100,
                            )
                          : 0
                      }%`,
            ...
            <Input
              ...
              helper={
                selectedType?.requires_document
                  ? 'Required for this leave type. PDF or image, up to 10 MB.'
                  : 'Optional. PDF or image, up to 10 MB.'
              }
            ...
            {estimatedDays > 0 && (
              <div className="flex items-center justify-between text-xs">
                ...
                <span
                  className={`font-mono tabular-nums font-medium ${selectedBalance && parseFloat(selectedBalance.remaining) < estimatedDays ? 'text-danger-fg' : 'text-primary'}`}
                >
