  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={data ? `${data.pending.length} pending · ${data.history.length} past` : undefined}
        actions={
          <Button
            variant="primary"
            size="sm"
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mt-1">
              {Array.from(
                { length: Math.max(0, Math.floor((maximumHours - minimumHours) / 0.5) + 1) },
                (_, i) => minimumHours + i * 0.5,
              ).map((h) => (
                <button
                  key={h}
                  type="button"
