...
export default function ChartOfAccountsPage() {
  ...
  const [statusTarget, setStatusTarget] = useState<{ account: Account; activate: boolean } | null>(
    null,
  );
  ...
  const statusMutation = useMutation({
    mutationFn: (target: { account: Account; activate: boolean }) =>
      target.activate
  ...
  return (
    <div>
        ...
        <EmptyState
          ...
          description={
            filters.search
              ...
              : 'Run the ChartOfAccountsSeeder to install the default 45-account COA.'
          }
        />
      )}
              ...
              {filteredData.map((root) => (
                <TreeRow
                  ...
                  onStatusChange={(account) =>
                    setStatusTarget({ account, activate: !account.is_active })
                  }
      ...
      <ConfirmDialog
        ...
        description={
          statusTarget ? `${statusTarget.account.code} · ${statusTarget.account.name}` : undefined
        }
...
export function TreeRow({
  ...
  return (
    <>
      <div
        className={cn(
          'group grid grid-cols-1 md:grid-cols-12 min-h-8 py-1.5 md:py-0 md:h-8 px-2.5 items-center border-b border-subtle hover:bg-subtle text-sm',
          !node.is_active && 'opacity-60',
        )}
      >
        <div className="col-span-1 font-mono tabular-nums text-muted">{node.code}</div>
        <div
          className="col-span-5 flex items-center gap-1.5"
        ...
        >
          ...
          <Link
            ...
            className={cn(
              'hover:text-accent hover:underline truncate',
        ...
        <div className="col-span-2 text-sm text-muted">
          {node.type_label ?? node.type} · {node.normal_balance_label ?? node.normal_balance}
        </div>
        <div className="col-span-2 text-right font-mono tabular-nums">
          {formatPeso(node.total_debit, '—')}
        </div>
        <div className="col-span-2 text-right font-mono tabular-nums font-medium">
          {formatPeso(node.current_balance, '—')}
        </div>
      </div>
      {isOpen &&
        hasChildren &&
