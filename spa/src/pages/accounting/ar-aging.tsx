  ...
  return (
    <div>
              ...
              <Button
                ...
                onClick={() =>
                  void downloadAuthenticatedFile(
                    statementsApi.csvUrl('ar-aging', { as_of: asOf }),
                    { errorMessage: 'Failed to export AR aging.' },
                  )
                }
              >
                CSV
      ...
      {isError && (
        <EmptyState
          icon="alert-circle"
          title="Failed to generate AR aging"
          action={
      ...
      {data && data.by_customer.length === 0 && (
        <EmptyState icon="inbox" title="No outstanding receivables" />
      )}
      {data && data.by_customer.length > 0 && (
              ...
              <table className={`${tableCls} min-w-[760px]`}>
                ...
                <tbody>
                  {data.by_customer.map((r) => (
                    <tr key={r.customer_id} className={trCls}>
                      ...
                      <Td
                        align="right"
                        mono
                        className={Number(r.d91_plus) > 0 ? ' text-danger-fg font-medium' : ''}
                      >
                        {formatPeso(r.d91_plus)}
                      </Td>
                  ...
                  <tr className={totalsTrCls}>
                    <Td>TOTAL</Td>
                    <Td align="right" mono>
                      {formatPeso(data.buckets.current)}
                    </Td>
                    <Td align="right" mono>
                      {formatPeso(data.buckets.d1_30)}
                    </Td>
                    <Td align="right" mono>
                      {formatPeso(data.buckets.d31_60)}
                    </Td>
                    <Td align="right" mono>
                      {formatPeso(data.buckets.d61_90)}
                    </Td>
                    <Td
                      align="right"
                      mono
                      className={Number(data.buckets.d91_plus) > 0 ? ' text-danger-fg' : ''}
                    >
                      {formatPeso(data.buckets.d91_plus)}
                    </Td>
                    <Td align="right" mono>
                      {formatPeso(data.buckets.total)}
                    </Td>
                  </tr>
                </tbody>
              </table>
