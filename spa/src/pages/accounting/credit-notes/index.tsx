...
const schema = z
  ...
  .refine((d) => d.type !== 'customer' || !!d.customer_id, {
    path: ['customer_id'],
    message: 'Customer required',
  })
  .refine((d) => d.type !== 'supplier' || !!d.vendor_id, {
    path: ['vendor_id'],
...
export default function CreditNotesPage() {
  ...
  const [filters, setFilters] = useUrlFilters<CreditNoteListParams & { search?: string }>({
    search: '',
    page: 1,
    per_page: 25,
  });
  ...
  const columns: Column<CreditNote>[] = [
    {
      key: 'number',
      header: 'CN no',
      cell: (r) => <span className="font-mono">{r.credit_note_number ?? 'DRAFT'}</span>,
    },
    ...
    },
    {
      key: 'status',
      header: 'Status',
      cell: (r) => <Chip variant={cnStatusVariant(r.status)}>{r.status_label}</Chip>,
    },
  ];
  ...
  const filterConfig: FilterConfig[] = [
    {
      ...
      label: 'Type',
      type: 'select',
      options: [{ value: '', label: 'All' }, ...(accountingOptions?.credit_note_types ?? [])],
    },
    {
      key: 'status',
      label: 'Status',
      type: 'select',
      options: [{ value: '', label: 'All' }, ...(accountingOptions?.credit_note_statuses ?? [])],
    },
  ];
...
function CreateCreditNoteModal({
  ...
  const { data: vendorsResp } = useQuery({
    ...
    queryFn: () => vendorsApi.list({ per_page: 200 }),
  ...
  const mutation = useMutation({
    mutationFn: (d: FormValues) =>
      ...
      } as Parameters<typeof creditNotesApi.create>[0]),
    onSuccess: (cn) => {
      toast.success(`Credit note drafted (${formatPeso(cn.total_amount)}).`);
      onCreated();
    },
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <Select label="Type" required {...register('type')} error={errors.type?.message}>
            ...
            {(accountingOptions?.credit_note_types ?? []).map((option) => (
          ...
          <Input
            ...
            placeholder="Price dispute, damaged goods, over-billing…"
          />
        </div>
        ...
        <div className="border border-default rounded-md overflow-hidden">
          <div className="hidden md:grid md:grid-cols-12 gap-2 h-row px-2.5 bg-subtle text-2xs uppercase tracking-wider text-muted font-medium border-b border-default items-center">
            <div className="col-span-5">
              {type === 'customer' ? 'Revenue account' : 'Expense account'}
            </div>
          ...
          {fields.map((field, idx) => (
            ...
            >
              <div className="col-span-5">
                <Select
                  {...register(`lines.${idx}.account_id` as const)}
              ...
              <div className="col-span-2">
                <Input
                  ...
                  {...numberInputProps()}
                  {...register(`lines.${idx}.amount` as const)}
                  error={errors.lines?.[idx]?.amount?.message}
                />
              </div>
        ...
        <div className="flex items-center justify-between">
          <Button
            ...
            icon={<LuPlus size={14} />}
            onClick={() =>
              append({ account_id: '', description: '', amount: undefined as unknown as number })
            }
          >
            Add line
          </Button>
