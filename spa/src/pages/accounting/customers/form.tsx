...
export default function CustomerFormPage({ mode }: { mode: 'create' | 'edit' }) {
  ...
  const { register, handleSubmit, setError, formState: { errors, isSubmitting } } = useForm<FormValues>({
    ...
    defaultValues: { payment_terms_days: 30, is_active: true } as FormValues,
    values: existing ? {
      ...
      <form onSubmit={handleSubmit((d) => mutation.mutate(d), onFormInvalid<FormValues>())} className="max-w-3xl mx-auto px-5 py-6 space-y-4">
        <Panel title="Identity">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input label="Payment terms (days)" type="number" min={0} max={365} className="font-mono tabular-nums text-right" {...register('payment_terms_days')} error={errors.payment_terms_days?.message} />
            <Input label="Credit limit" type="number" step="0.01" min="0" prefix="₱" className="font-mono tabular-nums text-right" {...register('credit_limit')} error={errors.credit_limit?.message} />
          </div>
