...
export default function CustomerDetailPage() {
  ...
  const invoiceColumns: Column<Invoice>[] = [
    ...
    },
    {
      key: 'status',
      header: 'Status',
      cell: (r) => <Chip variant={chipVariantForStatus(r.display_status)}>{r.display_status}</Chip>,
    },
  ];

  const overLimit = customer.credit_warning ?? false;
  const warningPct =
    customer.credit_warning_ratio == null ? null : Math.round(customer.credit_warning_ratio * 100);
  ...
  return (
    <div>
      ...
      <div className="px-5 py-4 grid grid-cols-3 gap-4">
        ...
        <StatCard
          label="Available"
          value={formatPeso(customer.credit_available)}
          delta={
            overLimit && warningPct !== null
              ? { value: `≥ ${warningPct}% used`, direction: 'down' }
              : undefined
          }
        />
      </div>
        ...
        <Panel title="Contact" className="col-span-1">
          <dl className="text-xs space-y-2">
            ...
            <div>
              ...
              <dd className="font-mono">{customer.tin ?? '—'}</dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd className="font-mono">
                {customer.payment_terms_days == null ? '—' : `${customer.payment_terms_days} days`}
              </dd>
            </div>
          </dl>
        </Panel>
      ...
      {can('crm.price_agreements.view') && (
        <div className="px-5 mt-4">
          <Panel
            ...
            meta={`${priceAgreements?.length ?? 0} ${(priceAgreements?.length ?? 0) === 1 ? 'agreement' : 'agreements'}`}
            noPadding
          >
            {!priceAgreements || priceAgreements.length === 0 ? (
              <div className="px-3 py-3 text-sm text-muted">
                No price agreements configured for this customer.
              </div>
            ) : (
                ...
                <tbody>
                  {priceAgreements.map((pa) => (
                    <tr
                      ...
                      onClick={() => navigate(`/crm/products/${pa.product?.id}`)}
                    >
                      <Td>
                        {pa.product ? (
                          <Link
                            to={`/crm/products/${pa.product.id}`}
                            onClick={(e) => e.stopPropagation()}
                            className="font-mono text-accent hover:underline"
                          >
                            {pa.product.part_number}
                          </Link>
                        ) : (
                        ...
                        <Chip
                          variant={
                            so.status === 'delivered' || so.status === 'invoiced'
                              ...
                              : so.status === 'cancelled'
                                ? 'danger'
                                : so.status === 'in_production' || so.status === 'confirmed'
                                  ? 'info'
