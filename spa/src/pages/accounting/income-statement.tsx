...
export default function IncomeStatementPage() {
  ...
  const yearStart = localIsoDate(new Date(today.getFullYear(), 0, 1));
  const yearEnd = localIsoDate(new Date(today.getFullYear(), 11, 31));
  // Dashboard drill-downs (?date_from=YYYY-MM-DD) seed the From date at mount.
  const [searchParams] = useSearchParams();
  const [from, setFrom] = useState(searchParams.get('date_from') ?? yearStart);
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          can('accounting.statements.export') && (
            <div className="flex gap-1.5">
              <Button
                ...
                onClick={() =>
                  void downloadAuthenticatedFile(
                    statementsApi.csvUrl('income-statement', { from, to }),
                    { errorMessage: 'Failed to export income statement.' },
                  )
                }
              >
                CSV
              ...
              <Button
                ...
                onClick={() =>
                  void downloadAuthenticatedFile(
                    statementsApi.pdfUrl('income-statement', { from, to }),
                    {
                      openInNewTab: true,
                      errorMessage: 'Failed to generate income statement PDF.',
                    },
                  )
                }
              >
      ...
      {isError && (
        <EmptyState
          icon="alert-circle"
          title="Failed to generate income statement"
          action={
              ...
              <table className={`${tableCls} min-w-[520px]`}>
                ...
                <tbody>
                  ...
                  <tr className={totalsTrCls}>
                    <Td>GROSS PROFIT</Td>
                    <Td align="right" mono>
                      {formatPeso(data.gross_profit)}
                    </Td>
                  </tr>
                  <Section
                    label="OPERATING EXPENSES"
                    rows={data.operating_expenses.accounts}
                  ...
                  <tr className={totalsTrCls}>
                    ...
                    <Td
                      ...
                      className={
                        ' text-base ' +
                        (Number(data.net_income) >= 0 ? 'text-success-fg' : 'text-danger-fg')
                      }
                    >
...
function Section({
  ...
  return (
    <>
      <tr>
        <Td
          className="bg-subtle text-2xs uppercase tracking-wider text-muted font-medium"
          colSpan={2}
        >
          {label}
        </Td>
