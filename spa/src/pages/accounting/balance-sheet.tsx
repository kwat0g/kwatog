  ...
  return (
    <div>
          ...
          can('accounting.statements.export') && (
            <div className="flex gap-1.5">
              <Button
                ...
                onClick={() =>
                  void downloadAuthenticatedFile(
                    statementsApi.csvUrl('balance-sheet', { as_of: asOf }),
                    { errorMessage: 'Failed to export balance sheet.' },
                  )
                }
              >
                CSV
              ...
              <Button
                ...
                onClick={() =>
                  void downloadAuthenticatedFile(
                    statementsApi.pdfUrl('balance-sheet', { as_of: asOf }),
                    { openInNewTab: true, errorMessage: 'Failed to generate balance sheet PDF.' },
                  )
                }
              >
                PDF
      ...
      <div className="px-5 py-3 border-b border-default flex items-end gap-3">
        ...
        />
        {data && (
          <Chip variant={data.balanced ? 'success' : 'danger'}>
            {data.balanced ? 'Balanced' : 'IMBALANCE'}
          </Chip>
        )}
      </div>
      ...
      {isError && (
        <EmptyState
          icon="alert-circle"
          title="Failed to generate balance sheet"
          action={
      ...
      {data && (
        <div className="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Section
            title="Liabilities"
            rows={data.liabilities.accounts}
          ...
          <div className="col-span-1 sm:col-span-2 lg:col-span-3 flex flex-wrap justify-end gap-5 pt-2 border-t border-default text-sm font-mono tabular-nums">
            <div>
              Total Assets: <span className="font-medium">{formatPeso(data.total_assets)}</span>
            </div>
            <div>
              Total Liabilities + Equity:{' '}
              <span className="font-medium">{formatPeso(data.total_liabilities_equity)}</span>
            </div>
          </div>
        </div>
      )}
...
function Section({
  ...
  return (
    <div className="border border-default rounded-md overflow-hidden">
      <div className="px-2.5 py-1.5 bg-subtle text-2xs uppercase tracking-wider text-muted font-medium border-b border-default">
        {title}
      </div>
      <div className="overflow-x-auto">
        <table className={`${tableCls} min-w-[520px]`}>
          ...
          <tbody>
            {rows.length === 0 && (
              <tr className={trCls}>
                <Td className="text-muted italic" colSpan={2}>
                  No movement
                </Td>
              </tr>
            )}
