...
const schema = z
  ...
  .superRefine((data, ctx) => {
    if (data.provenance_type === 'stock') {
      ...
      if (!data.goods_receipt_note_id)
        ctx.addIssue({
          code: 'custom',
          path: ['goods_receipt_note_id'],
          message: 'Accepted GRN is required',
        });
    } else {
      if (!data.exception_evidence?.trim())
        ctx.addIssue({
          code: 'custom',
          path: ['exception_evidence'],
          message: 'Evidence is required',
        });
      if (!data.exception_approved)
        ctx.addIssue({
          code: 'custom',
          path: ['exception_approved'],
...
export default function CreateBillPage() {
  ...
  const vatRateLabel = vatConfigured
    ? `${(Number(policies.vat_rate) * 100).toLocaleString()}%`
    : '—';
  ...
  useEffect(() => {
    ...
    if (selectedPo.vendor?.id)
      setValue('vendor_id', selectedPo.vendor.id, { shouldValidate: true, shouldDirty: true });
  ...
  const acceptedGrns = useMemo(
    () =>
      (selectedPo?.goods_receipt_notes ?? []).filter(
        (grn) => grn.status === 'accepted' || grn.status === 'partial_accepted',
      ),
    [selectedPo],
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setValue('due_date', localIsoDate(d), { shouldValidate: false, shouldDirty: true });
  ...
  const mutation = useMutation({
    mutationFn: (d: FormValues) =>
      billsApi.create({
        ...
        purchase_order_id:
          d.provenance_type === 'stock' ? d.purchase_order_id || undefined : undefined,
        goods_receipt_note_id:
          d.provenance_type === 'stock' ? d.goods_receipt_note_id || undefined : undefined,
        exception_evidence:
          d.provenance_type === 'service' ? d.exception_evidence?.trim() || undefined : undefined,
      ...
      >
        <Panel title="Header">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <Select
              label="Bill provenance"
              required
              {...register('provenance_type')}
              error={errors.provenance_type?.message}
            >
            ...
            {provenanceType === 'stock' ? (
              <>
                <Select
                  ...
                  helper="The accepted receipt and 3-way match are checked against this PO."
                >
                ...
                <Select
                  label="Accepted GRN"
                  required
                  {...register('goods_receipt_note_id')}
                  error={errors.goods_receipt_note_id?.message}
                  disabled={!purchaseOrderId}
                >
                  <option value="">— Select accepted GRN —</option>
                  {acceptedGrns.map((grn) => (
              ...
              <>
                <Textarea
                  ...
                  {...register('exception_evidence')}
                  error={errors.exception_evidence?.message}
                  helper="Contract, completion report, timesheet, or other evidence supporting this non-stock bill."
                />
                <div className="flex items-end">
                  <Switch
                    label="Approve exception"
                    description="I approve this service/non-stock exception and accept ownership of the evidence."
                    {...register('exception_approved')}
                  />
                </div>
              </>
            )}
            ...
            <div className="flex items-end">
              <Switch
                label={`VAT-able (${vatRateLabel})`}
                disabled={!vatConfigured}
                {...register('is_vatable')}
              />
            </div>
            <Textarea
        ...
        <Panel title="Line items">
          <div className="border border-default rounded-md overflow-hidden">
            ...
            {fields.map((field, idx) => {
              ...
              const lineTotal = ((Number(it.quantity) || 0) * (Number(it.unit_price) || 0)).toFixed(
                2,
              );
              return (
                <div
                ...
                >
                  ...
                  <div className="col-span-1 pt-1.5 text-right font-mono tabular-nums text-sm">
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <div className="text-sm font-mono tabular-nums">
              <div className="text-muted">
                Subtotal: <span className="text-primary">{formatPeso(totals.subtotal)}</span>
              </div>
              <div className="text-muted">
                VAT: <span className="text-primary">{formatPeso(totals.vat)}</span>
              </div>
        ...
        <FormActions>
          <Button type="button" variant="secondary" onClick={() => navigate('/accounting/bills')}>
            Cancel
          </Button>
          <Button
            type="submit"
