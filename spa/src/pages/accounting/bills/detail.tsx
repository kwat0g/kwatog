...
import { agingBucketLabel } from '@/lib/labels';
...
const MATCH_LINE_VARIANT: Record<MatchLineStatus, ChipVariant> = {
  ...
  both: 'warning',
  grn_short: 'danger',
  unmatched_bill_line: 'danger',
  duplicate_bill_line: 'danger',
};
...
export default function BillDetailPage() {
  ...
  const postMut = useMutation({
    mutationFn: (data?: { allow_override?: boolean; override_reason?: string }) =>
      billsApi.postDraft(id, data),
    onSuccess: () => {
  ...
  const payMut = useMutation({
    mutationFn: (d: PaymentFormValues) =>
      billsApi.recordPayment(
        id,
        {
  ...
  const approvePaymentMut = useMutation({
    ...
    onSuccess: () => {
      toast.success('Payment approval recorded.');
      qc.invalidateQueries({ queryKey: ['accounting', 'bills'] });
      void qc.refetchQueries({ queryKey: ['accounting', 'bills', id] });
    },
    onError: (e: Error & { response?: { data?: { message?: string } } }) =>
  ...
  // Pending payment requests reserve part of the balance and the API refuses
  // anything above the rest, so only offer and cap what is still unreserved.
  const unreserved = fromCentavos(
    toCentavos(bill.balance) -
      (bill.payments ?? [])
        .filter((p) => p.status === 'pending_approval')
        .reduce((sum, p) => sum + toCentavos(p.amount), 0),
  );
  const canRecordPayment = isOpen && toCentavos(unreserved) > 0;
  // Header accounts (e.g. 1000 Assets) share the 10xx prefix but cannot take a posting.
  const cashParentIds = new Set((cashAccounts?.data ?? []).map((a) => a.parent_id).filter(Boolean));
  const cashAccts = (cashAccounts?.data ?? []).filter(
    (a) => a.code.startsWith('10') && a.is_active && !cashParentIds.has(a.id),
  );
        ...
        actions={
          <div className="flex gap-1.5">
            <Button
              ...
              onClick={() =>
                void downloadAuthenticatedFile(billsApi.pdfUrl(bill.id), {
                  openInNewTab: true,
                  errorMessage: 'Failed to generate bill PDF.',
                })
              }
            >
              Print
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    ...
                    match?.overall_status === 'blocked'
                  ) {
                    if (can('accounting.bills.three_way_override')) setShowPostOverride(true);
                    else
                      toast.error(
                        'A finance checker with 3-way-match override permission must approve this variance.',
                      );
                  } else {
            ...
            {canRecordPayment && can('accounting.bills.pay') && (
              <Button
                variant="primary"
                size="sm"
                icon={<LuReceipt size={14} />}
 ...
 stay visible and clickable from here. */}
          <ChainHeader
            steps={buildP2pChain({
              pr: bill.purchase_order?.purchase_request
                ? {
                    id: bill.purchase_order.purchase_request.id,
                    number: bill.purchase_order.purchase_request.pr_number,
                  }
                : null,
              po: bill.purchase_order
                ? { id: bill.purchase_order.id, number: bill.purchase_order.po_number }
                : null,
      ...
      <div className="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        {bill.ewt_amount && bill.ewt_amount !== '0.00' && (
          <StatCard label="EWT" value={formatPeso(bill.ewt_amount)} />
        )}
        <StatCard label="Paid" value={formatPeso(bill.amount_paid)} />
        <StatCard
          label="Balance"
          value={formatPeso(bill.balance)}
          delta={bill.is_overdue ? { value: 'OVERDUE', direction: 'down' } : undefined}
        />
        <StatCard label="Aging" value={agingBucketLabel(bill.aging_bucket)} />
      </div>
        ...
        <div className="col-span-2 space-y-4">
          <Panel title="Details">
            <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
              <div>
                <dt className="text-2xs uppercase tracking-wider text-muted mb-0.5">Vendor</dt>
                <dd>{bill.vendor?.name}</dd>
              </div>
              <div>
                <dt className="text-2xs uppercase tracking-wider text-muted mb-0.5">Date</dt>
                <dd className="font-mono">{formatDate(bill.date)}</dd>
              </div>
              <div>
                <dt className="text-2xs uppercase tracking-wider text-muted mb-0.5">Due date</dt>
                <dd className="font-mono">{formatDate(bill.due_date)}</dd>
              </div>
              <div>
                ...
                <div>
                  ...
                  <dd>
                    <a
                      ...
                      href={`/purchasing/purchase-orders/${bill.purchase_order.id}`}
                    >
              ...
              {bill.three_way_overridden && bill.three_way_override_reason && (
                <div className="col-span-3">
                  ...
                  <dd className="text-warning-fg">{bill.three_way_override_reason}</dd>
                </div>
              )}
              {bill.three_way_overridden && bill.three_way_overridden_by && (
                <div>
                  ...
                  <dd>{bill.three_way_overridden_by.name}</dd>
                </div>
              )}
              {bill.journal_entry && (
                <div className="col-span-2">
                  ...
                  <dd>
                    <a
                      className="text-accent hover:underline font-mono"
                      href={`/accounting/journal-entries/${bill.journal_entry.id}`}
                    >
                      {bill.journal_entry.entry_number}
                    </a>{' '}
                    · {bill.journal_entry.status_label ?? bill.journal_entry.status}
                  </dd>
                </div>
              )}
            </dl>
              ...
              <table className={`${tableCls} min-w-[680px]`}>
                ...
                <tbody>
                  {bill.items?.map((i, idx) => (
                    <tr key={i.id} className={trCls}>
                      ...
                      <Td className="text-muted text-xs">
                        {i.expense_account ? (
                          <span>
                            <span className="font-mono">{i.expense_account.code}</span> ·{' '}
                            {i.expense_account.name}
                          </span>
                        ) : (
                          '—'
                        )}
                  ...
                  {bill.ewt_amount && bill.ewt_amount !== '0.00' && (
                    <tr className={trCls}>
                      <Td align="right" mono className="text-muted" colSpan={5}>
                        EWT withheld on payment (
                        {bill.ewt_rate ? `${(Number(bill.ewt_rate) * 100).toFixed(2)}%` : '—'})
                      </Td>
                      <Td align="right" mono className="text-danger-fg">
                        {formatPeso(bill.ewt_amount)}
                      </Td>
                    </tr>
                  )}
                  <tr className={totalsTrCls}>
                    <Td align="right" mono colSpan={5}>
                      Total
                    </Td>
                    <Td align="right" mono>
            ...
            <ul className="space-y-2 text-xs">
              {bill.payments!.map((p) => (
                <li key={p.id} className="border-b border-subtle pb-2">
                  ...
                  {p.ewt_amount && p.ewt_amount !== '0.00' && (
                    <div className="flex justify-between font-mono tabular-nums text-muted">
                      <span>EWT {formatPeso(p.ewt_amount)}</span>
                      <span>Cash {formatPeso(p.cash_amount ?? p.amount)}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between gap-2 text-muted">
                    ...
                    {/* Server decides: current step role and not the requester, not just the permission. */}
                    {p.status === 'pending_approval' &&
                      (p.can_decide ?? can('accounting.bills.payment_approve')) && (
                        <span className="flex gap-1">
                          <Button
                            type="button"
                            variant="ghost"
                            size="sm"
                            onClick={() => approvePaymentMut.mutate(p.id)}
                            loading={approvePaymentMut.isPending}
                          >
                            Approve
                          </Button>
                          <Button
                    ...
                    {p.status === 'posted' &&
                      can('accounting.bills.void_payment') &&
                      bill.status !== 'cancelled' && (
                        <Button
                          type="button"
                          variant="ghost"
          ...
          <Panel title="3-way match" className="col-span-3">
            {bill.three_way_review_status === 'manual_review' && (
              <div className="mb-3 rounded-md border border-danger/40 bg-danger-bg/20 px-3 py-2 text-sm text-danger-fg">
                Manual review is required before this draft can post. Confirm the variance against
                the supplier documents; an override requires an audit reason.
              </div>
            )}
            {matchLoading ? (
                  ...
                  <tbody>
                    {match.lines.map((l, idx) => {
                      ...
                      return (
                        <tr
                          key={`${l.item_id}-${idx}`}
                          className={cn(trCls, isBlock && 'bg-danger-bg/30')}
                        >
                          <Td>
                            {l.item_code && (
                          ...
                          <Td align="right" mono>
                            {formatPeso(l.po_unit_price)}
                          </Td>
                          <Td align="right" mono>
                            {l.grn_quantity_accepted}
                          </Td>
                          <Td align="right" mono>
                            {l.bill_quantity}
                          </Td>
                          <Td align="right" mono>
                            {formatPeso(l.bill_unit_price)}
                          </Td>
                          <Td align="right" mono>
                            {l.quantity_variance_pct.toFixed(2)}%
                          </Td>
                          <Td align="right" mono>
                            {l.price_variance_pct.toFixed(2)}%
                          </Td>
                          <Td>
                            <Chip variant={isBlock ? 'danger' : chip}>
      ...
      <Modal
        ...
        title={`Record payment for ${bill.bill_number}`}
        size="sm"
      >
        <form
          onSubmit={handleSubmit((d) => payMut.mutate(d), onFormInvalid<PaymentFormValues>())}
          className="space-y-3"
        >
          <Select
            label="Cash account"
          ...
          />
          <Input
            label={`Amount (max ${formatPeso(unreserved)})`}
            step="0.01"
            min="0.01"
            max={unreserved}
            ...
            helper={
              bill.ewt_amount && bill.ewt_amount !== '0.00'
                ? `Enter the payable settled. EWT is withheld pro-rata (${formatPeso(bill.ewt_amount)} on the full bill); the rest is paid out as cash.`
                : undefined
            }
          />
          <Select
          ...
          >
            <option value="">— Select —</option>
            {(accountingOptions?.payment_methods ?? []).map((option) => (
      ...
      <ReasonDialog
        ...
        onClose={() => {
          if (!rejectPaymentMut.isPending) setPaymentToReject(null);
        }}
      ...
      <ReasonDialog
        ...
        onClose={() => {
          if (!voidPaymentMut.isPending) setPaymentToVoid(null);
        }}
        onConfirm={(reason) => voidPaymentMut.mutate(reason)}
        title={
          paymentToVoid ? `Void payment of ${formatPeso(paymentToVoid.amount)}?` : 'Void payment?'
        }
