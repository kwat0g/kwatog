  ...
  .superRefine((data, ctx) => {
    ...
    if (customerReturn && !data.customer_id) {
      ctx.addIssue({
        code: 'custom',
        path: ['customer_id'],
        message: 'Select the customer returning the goods',
      });
    }
    if (!customerReturn && !data.vendor_id) {
      ctx.addIssue({
        code: 'custom',
        path: ['vendor_id'],
        message: 'Select the supplier the goods go back to',
      });
    }
    if (financeOnly && !data.finance_only_reason?.trim()) {
      ctx.addIssue({
        code: 'custom',
        path: ['finance_only_reason'],
        message: 'Explain why no goods are returning',
      });
    }
    ...
    data.items.forEach((line, index) => {
      if (financeOnly) {
        if (!line.product_id) {
          ctx.addIssue({
            ...
            message: 'Select the credited product',
          });
        }
        if (line.unit_price === undefined) {
          ctx.addIssue({
            ...
            message: 'Enter the credit unit price',
          });
        }
        return;
      }
      if (customerReturn && !line.item_id) {
        ctx.addIssue({
          ...
          message: 'Select the inventory item received back',
        });
      }
      if (customerReturn && !line.source_line) {
        ctx.addIssue({
          code: 'custom',
          path: ['items', index, 'source_line'],
          message: 'Select the invoice, order, or delivery line',
        });
      }
      if (!customerReturn && !line.item_id) {
        ctx.addIssue({
          code: 'custom',
          path: ['items', index, 'item_id'],
          message: 'Select the purchased item',
        });
      }
      if (!customerReturn && !line.source_line) {
        ctx.addIssue({
          code: 'custom',
          path: ['items', index, 'source_line'],
          message: 'Select the accepted GRN line',
        });
      }
    });
  });
...
export default function CreateReturnRequestPage() {
  ...
  useEffect(() => {
    ...
    reset({
      ...
      sales_order_id: existing.sales_order?.id ?? '',
      purchase_order_id: existing.purchase_order?.id ?? '',
      bill_id: existing.bill?.id ?? '',
      finance_only: existing.finance_only === true,
      finance_only_reason: existing.finance_only_reason ?? '',
      reason_code: existing.reason_code ?? '',
      reason_description: existing.reason_description ?? '',
      ...
      items: (existing.items ?? []).map((line) => ({
        ...
        source_line: sourceForExistingLine(line),
        bill_line: line.source_bill_item_id ? `bill_item:${line.source_bill_item_id}` : '',
  ...
  const { data: sourceOptions, isFetching: sourcesFetching } = useQuery({
    ...
    queryFn: () =>
      returnManagementApi.sourceOptions({
        type: returnType,
        ...(returnType === 'supplier_return' ? { vendor_id: partyId } : { customer_id: partyId }),
      }),
  ...
  const customerSourceLines = useMemo<SourceChoice[]>(
    () =>
      sourceOptions
        ? [
            ...sourceOptions.customer.invoices.flatMap((doc) =>
              doc.lines.map((line) => ({
                ...line,
                value: `invoice_item:${line.id}`,
                rootId: doc.id,
                sourceKind: 'invoice_item' as const,
                label: `Invoice ${doc.label} · ${line.label} · ${reservable(line)} available`,
              })),
            ),
            ...sourceOptions.customer.salesOrders.flatMap((doc) =>
              doc.lines.map((line) => ({
                ...line,
                value: `sales_order_item:${line.id}`,
                rootId: doc.id,
                sourceKind: 'sales_order_item' as const,
                label: `SO ${doc.label} · ${line.label} · ${reservable(line)} returnable of ${line.quantity} delivered`,
              })),
            ),
            ...sourceOptions.customer.deliveries.flatMap((doc) =>
              doc.lines.map((line) => ({
                ...line,
                value: `delivery_item:${line.id}`,
                rootId: doc.sales_order_id,
                sourceKind: 'delivery_item' as const,
                label: `Delivery ${doc.label} · ${line.label} · ${reservable(line)} returnable of ${line.quantity} delivered`,
              })),
            ),
          ]
        : [],
  ...
  const supplierSourceLines = useMemo<SourceChoice[]>(
    () =>
      sourceOptions
        ? sourceOptions.supplier.goodsReceipts.flatMap((doc) =>
            doc.lines.map((line) => ({
              ...line,
              value: `grn_item:${line.id}`,
              rootId: doc.purchase_order_id,
              sourceKind: 'grn_item' as const,
              label: `GRN ${doc.label} · ${line.label} · ${reservable(line)} returnable of ${line.quantity} accepted`,
            })),
          )
        : [],
    [sourceOptions],
  );
  const billLines = useMemo<SourceChoice[]>(
    () =>
      sourceOptions
        ? sourceOptions.supplier.bills.flatMap((doc) =>
            doc.lines.map((line) => ({
              ...line,
              value: `bill_item:${line.id}`,
              rootId: doc.id,
              sourceKind: 'grn_item' as const,
              label: `Bill ${doc.label} · ${line.label}`,
            })),
          )
        : [],
    [sourceOptions],
  ...
  const selectSourceLine = (index: number, value: string) => {
    ...
    if (!choice) return;
    if (isSupplierReturn) {
      setValue('purchase_order_id', choice.rootId ?? '');
      setValue(`items.${index}.item_id`, choice.item_id ?? '');
      setValue(`items.${index}.lot_number`, choice.lot_number ?? '');
    } else if (choice.sourceKind === 'invoice_item') {
      setValue('invoice_id', choice.rootId ?? '');
      setValue('sales_order_id', '');
      setValue(`items.${index}.product_id`, choice.product_id ?? '');
    } else {
      setValue('sales_order_id', choice.rootId ?? '');
      setValue('invoice_id', '');
      setValue(`items.${index}.product_id`, choice.product_id ?? '');
    }
  };
  const selectBillLine = (index: number, value: string) => {
    ...
    setValue(`items.${index}.bill_line`, value);
    setValue('bill_id', choice?.rootId ?? '');
  };
  ...
  const mutation = useMutation({
    mutationFn: (data: FormValues) => {
      ...
      const payload: ReturnRequestFormData = {
        type: data.type as ReturnRequestFormData['type'],
        return_date: data.return_date,
        customer_id: customer ? data.customer_id || undefined : undefined,
        vendor_id: customer ? undefined : data.vendor_id || undefined,
        invoice_id: customer && !data.finance_only ? data.invoice_id || undefined : undefined,
        sales_order_id:
          customer && !data.finance_only ? data.sales_order_id || undefined : undefined,
        purchase_order_id: !customer ? data.purchase_order_id || undefined : undefined,
        bill_id: !customer ? data.bill_id || undefined : undefined,
        finance_only: customer ? data.finance_only === true : false,
        finance_only_reason:
          customer && data.finance_only ? data.finance_only_reason || undefined : undefined,
        ...
        items: data.items.map((line) => {
          ...
          return {
            ...
            source_sales_order_item_id:
              source?.sourceKind === 'sales_order_item' ? source.id : undefined,
            ...
            source_po_item_id:
              source?.sourceKind === 'grn_item' ? source.po_item_id || undefined : undefined,
            source_bill_item_id: bill?.value.replace('bill_item:', '') || undefined,
          };
        }),
      };
      return editing
        ? returnManagementApi.update(id!, payload)
        : returnManagementApi.create(payload);
    },
    onSuccess: (rma) => {
      queryClient.invalidateQueries({ queryKey: ['return-management'] });
      queryClient.invalidateQueries({ queryKey: ['return-request', id] });
      toast.success(editing ? 'Draft return request updated.' : 'Return request created.');
      navigate(`/return-management/${rma.id}`);
    },
    onError: (error) =>
      applyServerValidationErrors<FormValues>(
        error,
        setError,
        editing ? 'Failed to update return request.' : 'Failed to create return request.',
      ),
  });
  ...
  return (
    <div>
      <PageHeader
        title={editing ? 'Edit Return Request' : 'New Return Request'}
        subtitle={
          editing ? 'Correct a draft before approval' : 'Create a customer or supplier return'
        }
        backTo={editing ? `/return-management/${id}` : '/return-management'}
      />
      <FormDraftBanner safety={safety} />
      <form
        onSubmit={handleSubmit((data) => mutation.mutate(data), onFormInvalid<FormValues>())}
      ...
      >
        <Panel title="Type & Source">
          ...
          {returnType === 'customer_return' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
              ...
              <label className="flex items-center gap-2 text-sm self-end pb-2">
                <input type="checkbox" {...register('finance_only')} /> Finance-only credit — no
                goods returning
              </label>
            </div>
          )}
          ...
          {returnType && partyId && !financeOnly && (
            <p className="text-xs text-muted mt-3">
              {sourcesFetching
                ? 'Loading party-scoped source lines…'
                : 'Each stockable line must be linked to a delivered or accepted source line below.'}
            </p>
          )}
          {financeOnly && (
            <div className="mt-3">
              <Textarea
                label="Finance-only reason"
                required
                rows={2}
                placeholder="Why is no inventory returning?"
                {...register('finance_only_reason')}
                error={errors.finance_only_reason?.message}
              />
            </div>
          )}
        </Panel>
        ...
        >
          {fields.length === 0 ? (
            <div className="text-muted text-sm py-2">
              No items added yet. Click “Add Item” to add the returned line.
            </div>
          ) : (
            <div className="space-y-3">
              {fields.map((field, index) => {
                ...
                return (
                  <div key={field.id} className="border border-default rounded-md p-3 space-y-2">
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-start">
                      <div className="md:col-span-3">
                        {financeOnly ? (
                          <Select
                            label="Credited product"
                          ...
                          <Select
                            ...
                            error={itemError?.item_id?.message}
                          >
                      ...
                      {!financeOnly && (
                        <div className="md:col-span-4">
                          <Select
                            label={isSupplierReturn ? 'Accepted GRN line' : 'Source line'}
                            {...register(`items.${index}.source_line` as const)}
                            error={itemError?.source_line?.message}
                            onChange={(event) => {
                              setValue(`items.${index}.source_line`, event.target.value, {
                                shouldValidate: true,
                              });
                              selectSourceLine(index, event.target.value);
                            }}
                          >
                            <option value="">
                              {sourcesFetching ? 'Loading source lines…' : '— Select source line —'}
                            </option>
                      ...
                      {isSupplierReturn && (
                        <div className="md:col-span-3">
                          <Select
                            label="Bill line (optional)"
                            {...register(`items.${index}.bill_line` as const)}
                            onChange={(event) => selectBillLine(index, event.target.value)}
                          >
                            <option value="">— No bill line —</option>
                            {billLines.map((choice) => (
                              <option key={choice.value} value={choice.value}>
                                {choice.label}
                              </option>
                            ))}
                      ...
                      <div className="md:col-span-2">
                        <Input
                          label={financeOnly ? 'Unit price' : 'Price (source)'}
                          type="number"
                          step="0.01"
                          min="0"
                          className="font-mono tabular-nums text-right"
                          {...register(`items.${index}.unit_price` as const)}
                          error={itemError?.unit_price?.message}
                          placeholder={financeOnly ? undefined : 'From source'}
                        />
                      </div>
                    ...
                    {isSupplierReturn && (
                      <p className="text-xs text-muted">
                        Selecting a GRN line fills the PO line, item, and controlled lot provenance
                        automatically.
                      </p>
                    )}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      <Select label="Condition" {...register(`items.${index}.condition` as const)}>
                      ...
                      <Input
                        label="Serial number"
                        placeholder="Optional"
                        {...register(`items.${index}.serial_number` as const)}
                      />
                    </div>
                  </div>
                );
        ...
        <FormActions>
          <Button
            type="button"
            variant="secondary"
            onClick={() => navigate(editing ? `/return-management/${id}` : '/return-management')}
          >
            Cancel
          </Button>
          <Button
