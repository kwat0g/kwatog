...
export default function ReturnRequestDetailPage() {
  ...
  const [confirm, setConfirm] = useState<
    'submit' | 'approve' | 'receive' | 'inspect' | 'complete' | 'cancel' | null
  >(null);
  ...
  const reasonLabel = new Map(
    (options?.reasons ?? []).map((option) => [option.value, option.label]),
  );
  const resolutionLabel = new Map(
    (options?.resolutions ?? []).map((option) => [option.value, option.label]),
  );
  const conditionLabel = new Map(
    (options?.conditions ?? []).map((option) => [option.value, option.label]),
  );
  const dispositionLabel = new Map(
    (options?.dispositions ?? []).map((option) => [option.value, option.label]),
  );
  ...
  const submitMut = useMutation({
    ...
    onSuccess: () => {
      invalidate();
      toast.success('RMA submitted for approval.');
      setConfirm(null);
    },
  ...
  const inspectMut = useMutation({
    ...
    onSuccess: (updated) => {
      invalidate();
      toast.success(
        updated.inspection_handoff?.status === 'manual_required'
  ...
  const availableActions = (
    status?: string,
  ): Array<{ key: string; label: string; variant?: 'primary' | 'danger' | 'default' }> => {
    if (!status) return [];
    switch (status) {
      case 'draft':
        return [
      ...
      case 'inspected': {
        ...
        return [
          ...
          ...(disposed
            ? []
            : [{ key: 'dispose', label: 'Dispose Items', variant: 'primary' as const }]),
          ...(disposed
  ...
  > = {
    submit: {
      title: 'Submit RMA for approval?',
      description:
        'The RMA enters the approval chain. Edits are no longer allowed until approved or rejected.',
      label: 'Submit',
      variant: 'primary',
    },
    approve: {
      title: 'Approve this RMA?',
      description: 'Approval is recorded against your account in the audit log.',
      label: 'Approve',
      variant: 'primary',
    },
    receive: {
      title: 'Record receipt of returned items?',
      description: 'Marks the items as physically received and ready for inspection.',
    ...
    cancel: {
      title: 'Cancel this RMA?',
      description: 'Cancellation is permanent. The RMA cannot be reopened.',
      label: 'Yes, cancel RMA',
      variant: 'danger',
    },
  };
            ...
            {rma.inspection_handoff?.status === 'manual_required' && canManage && (
              <Button
                ...
                icon={
                  <LuRefreshCw
                    size={13}
                    className={retryInspectionMut.isPending ? 'animate-spin' : ''}
                  />
                }
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-4">
            ...
            <Panel title="RMA Details">
              <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 text-sm mt-2">
                ...
                <div>
                  ...
                  <dd className="flex flex-col gap-0.5">
                    ...
                    {rma.sales_order && (
                      <Link
                        to={`/crm/sales-orders/${rma.sales_order.id}`}
                    ...
                    {rma.invoice && (
                      <Link
                        to={`/accounting/invoices/${rma.invoice.id}`}
                        className="text-accent hover:underline font-mono"
                      >
                    ...
                    {rma.purchase_order && (
                      <Link
                        to={`/purchasing/purchase-orders/${rma.purchase_order.id}`}
                        className="text-accent hover:underline font-mono"
                      >
                    ...
                    {rma.bill && (
                      <Link
                        to={`/accounting/bills/${rma.bill.id}`}
                        className="text-accent hover:underline font-mono"
                      >
              ...
              {rma.customer_notes && (
                <div className="mt-3">
                  <div className="text-2xs uppercase tracking-wider text-muted mb-0.5">
                    Customer Notes
                  </div>
              ...
              <div className="flex items-start gap-3 rounded-md border border-warning/40 bg-warning-bg/10 px-4 py-3 text-sm">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-muted">
                    {rma.inspection_handoff.message ||
                          ...
                          {item.disposition ? (
                            ...
                            >
                              {item.disposition_label ??
                                dispositionLabel.get(item.disposition) ??
                                item.disposition}
                            </Chip>
                          ) : (
                            '—'
                ...
                {timeline
                  ...
                  .map((t) => (
                    <div key={t.key} className="flex items-center gap-2">
                      <div
                        className={`h-2 w-2 rounded-full shrink-0 ${TIMELINE_DOT[t.key] ?? 'bg-strong'}`}
                      />
  ...
  auto-bill / auto-invoice review-then-post pattern). */}
                  ...
                  <div className="flex items-center gap-3 rounded-md border border-success/40 bg-success-bg/10 px-4 py-3 text-sm">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-muted">
                        ...
                        <Link
                          to={`/accounting/credit-notes/${rma.credit_note.id}`}
                          className="font-mono text-accent hover:underline"
                        >
                          {rma.credit_note.credit_note_number ?? '(draft)'}
                        </Link>{' '}
                        · {formatPeso(rma.credit_note.total_amount)}. Review and finalize to post
                        the AR credit to the GL.
                      </div>
                    </div>{' '}
                    {can('accounting.credit_notes.manage') && (
                      <Button
                        variant="secondary"
                        size="sm"
                        icon={<LuFileText size={14} />}
                        onClick={() => setFinalizeCnId(rma.credit_note!.id)}
                        loading={finalizeCn.isPending}
                      >
                        Finalize
                      </Button>
                    )}
                    ...
                    <div className="flex-1">
                      <div className="font-medium">
                        {rma.type === 'supplier_return'
                          ...
                          : 'Goods restocked into inventory'}
                      </div>
                      <div className="text-muted">
                        {rma.type === 'supplier_return'
                  ...
                  <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 text-sm mt-2">
                      ...
                      <dd>
                        {rma.credit_note ? (
                          <Link
                            ...
                            className="text-accent hover:underline font-mono"
                          >
                            {rma.credit_note.credit_note_number} ·{' '}
                            {formatPeso(rma.credit_note.total_amount)}
                          </Link>
                        ) : (
                    ...
                    <div>
                      ...
                      <dd>
                        {rma.replacement_purchase_order ? (
                          <Link
                            to={`/purchasing/purchase-orders/${rma.replacement_purchase_order.id}`}
                            className="text-accent hover:underline font-mono"
                          >
                      ...
                      <dd>
                        <Chip
                          variant={
                            rma.inspection_handoff?.status === 'manual_required'
                              ? 'warning'
                              : rma.inspection_handoff?.status === 'generated'
                                ? 'success'
                                : 'neutral'
                          }
                        >
                          {rma.inspection_handoff?.status_label ??
                            rma.inspection_handoff?.status ??
                            '—'}
                        </Chip>
                      </dd>
                    </div>
     ...
     Receive and complete have their own dialogs — they need input. */}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (finalizeCnId) finalizeCn.mutate(finalizeCnId);
        }}
      ...
      <Modal isOpen={confirm === 'complete'} onClose={() => setConfirm(null)} title="Complete RMA">
        {' '}
