...
} from '@/api/attendance/attendances';
...
import { formatTime, formatDate, localIsoDate } from '@/lib/formatDate';
...
function AttendanceCorrectionModal({
  ...
  const mutation = useMutation({
    ...
    onError: (error) => {
      ...
      const general =
        payload?.errors?.error?.[0] ??
        (payload?.message && !payload.errors ? payload.message : undefined);
      if (general) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEdit ? 'Correct attendance record' : 'Add manual attendance'}
      size="lg"
    >
      <form
        onSubmit={handleSubmit(
          (values) => mutation.mutate(values),
          onFormInvalid<AttendanceFormValues>(),
        )}
        className="space-y-4"
      >
        {errors.root?.message && (
          <div
            role="alert"
            className="rounded-md border border-danger bg-danger/10 px-3 py-2 text-sm text-danger-fg"
          >
            {errors.root.message}
          </div>
        )}
        ...
        <Textarea
          ...
          placeholder="Reason or source of the correction"
        />
        <ModalFooter>
...
export default function AttendancePage() {
  ...
  const { data: employeeData } = useQuery({
    ...
    queryFn: () =>
      employeesApi.list({ status: 'active', per_page: 100, sort: 'last_name', direction: 'asc' }),
    enabled: canEdit && correctionOpen,
  });
  const { data: shiftData } = useQuery({
    queryKey: ['attendance', 'shifts', 'attendance-options'],
    queryFn: () =>
      shiftsApi.list({ is_active: true, per_page: 100, sort: 'name', direction: 'asc' }),
    enabled: canEdit && correctionOpen,
  });
  ...
  const columns: Column<Attendance>[] = [
    ...
    },
    {
      ...
      sortable: true,
      align: 'right',
      cell: (r) => <NumCell>{r.regular_hours}</NumCell>,
    },
    {
      key: 'overtime_hours',
      header: 'OT',
      sortable: true,
      align: 'right',
      cell: (r) => <NumCell>{r.overtime_hours}</NumCell>,
    },
    {
      key: 'night_diff_hours',
      header: 'ND',
      align: 'right',
      cell: (r) => <NumCell>{r.night_diff_hours}</NumCell>,
    },
    {
      key: 'tardiness_minutes',
      header: 'Tardy',
      align: 'right',
      cell: (r) => <NumCell className="text-warning-fg">{minToHm(r.tardiness_minutes)}</NumCell>,
    },
    {
      key: 'status',
      header: 'Status',
      cell: (r) => (
        <Chip variant={chipVariantForStatus(r.status)}>
          {r.status_label ?? statusLabels.get(r.status) ?? r.status}
        </Chip>
      ),
    },
  ];
  ...
  const filterConfig: FilterConfig[] = [
      ...
      : []),
    {
      ...
      label: 'Status',
      type: 'select',
      options: [{ value: '', label: 'All' }, ...(attendanceOptions?.statuses ?? [])],
    },
  ];
    ...
    <div>
          ...
          <>
            ...
            {(can('attendance.edit') || can('attendance.shifts.manage')) && (
              <Button
                ...
                onClick={() => navigate('/hr/attendance/shifts')}
              >
                Shifts
              </Button>
            )}
            {(can('attendance.edit') || can('attendance.holidays.manage')) && (
              <Button
                ...
                size="sm"
                icon={<LuSun size={14} />}
                onClick={() => navigate('/hr/attendance/holidays')}
              >
                Holidays
              </Button>
            )}
            {can('attendance.import') && (
              <Button
                ...
                onClick={() => navigate('/hr/attendance/import')}
              >
                Import DTR
              </Button>
            )}
      ...
      {data && data.data.length === 0 && (
        <EmptyState
          ...
          description={
            filters.search ? 'Try a different search.' : 'Import a biometric CSV to get started.'
          }
          action={
            canEdit ? (
              ...
              <Button variant="primary" onClick={() => navigate('/hr/attendance/import')}>
                Import DTR
              </Button>
            ) : undefined
          }
