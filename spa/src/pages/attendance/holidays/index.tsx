...
export default function HolidaysPage() {
 ...
 const [pendingRestore, setPendingRestore] = useState<Holiday | null>(null);
 const [scope, setScope] = useState<ArchiveScope>('active');

 const { data, isLoading, isError, refetch } = useQuery({
 queryKey: ['attendance', 'holidays', year, { trashed: archiveToTrashed(scope) }],
 queryFn: () => holidaysApi.list({ year, per_page: 200, trashed: archiveToTrashed(scope) }),
 ...
 mutationFn: (id: string) => holidaysApi.restore(id),
 onSuccess: () => {
 qc.invalidateQueries({ queryKey: ['attendance', 'holidays'] });
 ...
 onRestore={(h) => setPendingRestore(h)}
 showRestore={scope === 'only'}
 />
 )}
 ...
 description={<>Archive <span className="font-medium">{pendingDelete.name}</span> on {formatDate(pendingDelete.date)}? It can be restored later.</>}
 variant="danger"
 ...
 description={<>Restore <span className="font-medium">{pendingRestore.name}</span>? It will reappear in active lists.</>}
...
function ListView({
 holidays, canManage, onEdit, onDelete, onRestore, showRestore,
}: {
 holidays: Holiday[];
 canManage: boolean;
 onEdit: (h: Holiday) => void;
 onDelete: (h: Holiday) => void;
 onRestore: (h: Holiday) => void;
 showRestore: boolean;
}) {
