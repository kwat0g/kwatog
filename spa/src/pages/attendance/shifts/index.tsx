...
export default function ShiftsPage() {
 ...
 queryKey: ['attendance', 'shifts', filters, { trashed: archiveToTrashed(scope) }],
 queryFn: () => shiftsApi.list({ ...filters, trashed: archiveToTrashed(scope) }),
 ...
 toast.error(e.response?.data?.message ?? 'Failed to archive shift.');
 },
 });

 const restoreMutation = useMutation({
 mutationFn: (id: string) => shiftsApi.restore(id),
 onSuccess: () => {
 qc.invalidateQueries({ queryKey: ['attendance', 'shifts'] });
 ...
 onError: (e: AxiosError<{ message?: string }>) => {
 toast.error(e.response?.data?.message ?? 'Failed to restore shift.');
 },
 });
