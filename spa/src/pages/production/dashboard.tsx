  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Button variant="secondary" size="sm" onClick={() => navigate('/production/oee')}>
            OEE Report
          </Button>
        }
      />
      <div className="px-5 py-4 space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard
            ...
            helper={
              k.today_output_reject > 0 ? `${formatInt(k.today_output_reject)} rejects` : undefined
            }
          />
          <StatCard label="Active work orders" value={formatInt(k.active_work_orders)} />
          <StatCard
                  ...
                  <div key={s.label}>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className="font-mono tabular-nums text-muted">
                        {s.count} <span className="text-2xs">({s.percent.toFixed(1)}%)</span>
                      </span>
                    </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <Panel
            title={`Defect Pareto (${data.defect_history_days}d)`}
            meta={`top ${data.defect_pareto.length}`}
          >
            {data.defect_pareto.length === 0 ? (
              <div className="text-sm text-muted">
                No defects recorded in the selected history window.
              </div>
            ) : (
              <div className="space-y-2">
                {data.defect_pareto.map((d) => (
                  <div key={d.defect_code}>
                    <div className="flex justify-between text-xs mb-1">
                      <span>
                        <span className="font-mono">{d.defect_code}</span> ·{' '}
                        <span className="text-muted">{d.defect_name}</span>
                      </span>
                      <span className="font-mono tabular-nums">
                        {formatInt(d.count)}{' '}
                        <span className="text-2xs text-muted">({d.percent.toFixed(1)}%)</span>
                      </span>
                    </div>
