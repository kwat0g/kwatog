...
export default function WarehouseMapPage() {
 ...
 const setView = (v: 'map' | 'count') => {
  ...
  navigate(v === 'count' ? '/inventory/warehouse-map?view=count' : '/inventory/warehouse-map', { replace: true });
 };
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-2xs text-muted">
       ...
       {statusLegend.map(([key, label]) => (
        <span key={key} className="inline-flex items-center gap-1.5">
         <span aria-hidden className={cn('h-2.5 w-2.5 rounded-[3px]', STATUS_SWATCH[key])} />
         {label}
        </span>
       ))}
      </div>
       ...
       <>
        ...
        {data.length > 1 && (
         <SegmentedControl
          ...
          label="Warehouse"
          value={wh?.id ?? ''}
          onChange={(id) => { setActiveWh(id); setActiveZone(null); setSelectedBin(null); setBinQuery(''); }}
        ...
        <div className="grid grid-cols-12 gap-4">
         ...
         <div className="col-span-3 space-y-1">
          ...
          {zones.map((z) => (
           <button
            key={z.id}
            type="button"
            onClick={() => { setActiveZone(z.id); setSelectedBin(null); setBinQuery(''); }}
            className={`w-full text-left px-2 py-1.5 text-xs rounded-md transition-colors cursor-pointer border ${focusRingInset} ${
             zone?.id === z.id ? 'bg-accent/10 text-accent border-accent/30 shadow-sm' : 'bg-surface border-default text-secondary hover:border-subtle hover:text-primary hover:bg-elevated'
            }`}
           >
         ...
         <div className="col-span-6">
          ...
          {zone && visibleBins.length === 0 && (
           <div className="text-sm text-muted py-5 text-center">No bins match “{binQuery}”.</div>
          )}
         </div>
