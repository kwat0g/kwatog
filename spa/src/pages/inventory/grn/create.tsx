...
export default function CreateGrnPage() {
  ...
  const { data: openPos } = useQuery({
    queryKey: ['inventory', 'grn', 'receivable-pos'],
    queryFn: () => grnApi.receivablePurchaseOrders(),
  });
  const poList = openPos?.data ?? [];

  const { data: po } = useQuery({
    queryKey: ['inventory', 'grn', 'receivable-pos', poId],
    queryFn: () => grnApi.receivablePurchaseOrder(poId),
    enabled: !!poId,
  });
      ...
      setItems(
        po.items
          ...
          .map((l) => ({
            ...
            // RFQ lines receive at delivered cost (freight/charges capitalized).
            unit_cost: l.delivered_unit_cost ?? l.unit_price,
            received_uom_code: '',
            lot_number: '',
  ...
  const validate = (): boolean => {
    ...
    items.forEach((it, idx) => {
      ...
      if (qty > 0) {
        if (qty > Number(it.remaining))
          errs.quantity = `Cannot receive more than remaining (${it.remaining}).`;
  ...
  const m = useMutation({
    mutationFn: (d: CreateGrnData) => grnApi.create(d, idempotencyKey.current),
    onSuccess: (g) => {
    ...
    <div>
      ...
      <div className="max-w-5xl mx-auto px-5 py-4 space-y-4">
                  ...
                  {items.map((line, i) => (
                    <Fragment key={line.purchase_order_item_id}>
                      <tr key={line.purchase_order_item_id} className={cn(trCls, 'align-top')}>
                        ...
                        <Td align="right" mono>
                          <Input
                            ...
                            onChange={(e) =>
                              setItems(
                                items.map((it, k) =>
                                  k === i ? { ...it, quantity_received: e.target.value } : it,
                                ),
                              )
                            }
                        ...
                        <Td align="right" mono>
                          <span className="font-mono tabular-nums">{line.unit_cost}</span>
                        </Td>
                        <Td>
                          <Select
                            fieldSize="sm"
                            ...
                            onChange={(e) =>
                              setItems(
                                items.map((it, k) =>
                                  k === i ? { ...it, location_id: e.target.value } : it,
                                ),
                              )
                            }
                        ...
                        <Td colSpan={7}>
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2">
                            <Input
                              ...
                              value={line.received_uom_code}
                              onChange={(e) =>
                                setItems(
                                  items.map((it, k) =>
                                    k === i ? { ...it, received_uom_code: e.target.value } : it,
                                  ),
                                )
                              }
                            />
                            <Input
                              ...
                              onChange={(e) =>
                                setItems(
                                  items.map((it, k) =>
                                    k === i ? { ...it, lot_number: e.target.value } : it,
                                  ),
                                )
                              }
                            />
                            <Input
                              ...
                              label="Supplier lot"
                              maxLength={100}
                              value={line.supplier_lot_reference}
                              onChange={(e) =>
                                setItems(
                                  items.map((it, k) =>
                                    k === i
                                      ? { ...it, supplier_lot_reference: e.target.value }
                                      : it,
                                  ),
                                )
                              }
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setItems(
                                  items.map((it, k) =>
                                    k === i ? { ...it, expiry_date: e.target.value } : it,
                                  ),
                                )
                              }
                            />
                            <Input
                              ...
                              label="Moisture %"
                              inputMode="decimal"
                              value={line.moisture_percentage}
                              onChange={(e) =>
                                setItems(
                                  items.map((it, k) =>
                                    k === i ? { ...it, moisture_percentage: e.target.value } : it,
                                  ),
                                )
                              }
                            />
                            <Input
                              ...
                              label="COA path"
                              maxLength={500}
                              value={line.coa_document_path}
                              onChange={(e) =>
                                setItems(
                                  items.map((it, k) =>
                                    k === i ? { ...it, coa_document_path: e.target.value } : it,
                                  ),
                                )
                              }
                            />
                            <div className="text-2xs text-muted self-center">
                              COA verification is completed by Quality after receipt.
                            </div>
                          </div>
                        </Td>
                      </tr>
        ...
        <div className="flex justify-end gap-2">
          <Button variant="secondary" onClick={() => nav('/inventory/grn')} disabled={m.isPending}>
            Cancel
          </Button>
          <Button
            variant="primary"
            onClick={submit}
            disabled={!poId || items.length === 0 || m.isPending}
            loading={m.isPending}
          >
            Create GRN
          </Button>
          ...
          po ? (
            <>
              ...
              <span className="font-mono font-medium text-primary">
                {items.filter((i) => Number(i.quantity_received) > 0).length}
              </span>{' '}
              line(s) against PO{' '}
              <span className="font-mono font-medium text-primary">{po.po_number}</span>.
              <br />
              The GRN starts in <span className="font-medium">pending QC</span>; stock will only
