...
import type { AuditLogEntry } from '@/api/admin/audit-logs';
...
function UsersTab() {
  ...
  if (isError || !data?.data?.length) {
    return (
      <EmptyState icon="users" title="No users found" description="Create a user to get started." />
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="overflow-x-auto">
          <table className={tableCls}>
            ...
            <tbody>
              {data.data.map((u: AdminUserListItem) => (
                <tr
                  key={u.id}
                ...
                >
                  ...
                  <Td className="text-secondary">{formatDateIso(u.created_at)}</Td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="flex gap-3">
        ...
        <Link to="/admin/users/create" className="text-sm text-accent hover:underline">
          Create user →
        </Link>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      <Panel
        title="Roles"
        actions={
          <Link to="/admin/roles" className="text-sm text-accent hover:underline">
            View all →
          </Link>
        }
      >
              ...
              {data.data.map((r: Role) => (
                ...
                >
                  ...
                  <Td className="text-secondary">
                    {r.last_modified_by
                      ? `${r.last_modified_by} · ${formatDateIso(r.last_modified_at, '')}`
                      : '—'}
                  </Td>
                </tr>
              ))}
      ...
      <div className="flex gap-3">
        ...
        <Link to="/admin/roles/create" className="text-sm text-accent hover:underline">
          Create role →
        </Link>
        <Link to="/admin/roles/compare" className="text-sm text-accent hover:underline">
          Compare roles →
        </Link>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="mb-3 text-xs text-text-subtle">
        {totalPerms} total permissions across {modules.length} modules
      </div>
    ...
    <div className="space-y-4">
      ...
      >
        <div className="space-y-2">
          {data.data.slice(0, 10).map((entry: AuditLogEntry) => (
            <div
              key={entry.id}
      ...
      <div className="flex gap-3">
        ...
        <Link to="/admin/activity" className="text-sm text-accent hover:underline">
          Activity feed →
        </Link>
      </div>
    </div>
