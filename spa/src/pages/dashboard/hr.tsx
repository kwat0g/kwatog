...
function AttendanceSummaryPanel({
  ...
  return (
    <Panel
      ...
      actions={
        <Link className="text-xs text-link hover:underline" to="/hr/attendance">
          Open →
        </Link>
      }
    >
      ...
      {data.total === 0 ? (
        ...
        <>
          ...
          <div className="space-y-1.5">
            {items.map((i) => (
              <div key={i.label} className="flex items-center justify-between text-sm">
                <Link to={i.href} className="hover:underline">
                  {i.label}
                </Link>
...
function DepartmentHeadcountPanel({
  ...
  return (
    <Panel
      ...
      actions={
        <Link className="text-xs text-link hover:underline" to="/hr/employees">
          Employees →
        </Link>
      }
    >
    ...
    >
      {alerts.length === 0 ? (
        <EmptyState
          size="compact"
          icon="check-circle"
          title="No probation reviews due"
          description={`No probationary employees end probation in the next ${horizonDays} days.`}
        />
      ) : (
...
function LeaveCalendarPanel({
   ...
   * calendar week, and it runs FORWARD from today — so a manager reading it as
   * "this week" mis-plans coverage the moment the setting is anything but 7.
   */
  horizonDays?: number;
}) {
  return (
    <Panel
      ...
      meta={<PeriodNote days={horizonDays} prefix="Next" direction="future" />}
      actions={
        <Link className="text-xs text-link hover:underline" to="/hr/leaves">
          All leaves →
        </Link>
      }
    >
      {leaves.length === 0 ? (
        <EmptyState
          ...
          icon="calendar"
          title="Everyone in"
          description="No approved leaves in this window."
        />
      ) : (
        <div className="space-y-1.5 text-sm">
          {leaves.map((l) => (
...
function CalendarEventsPanel({
  ...
  return (
    <Panel
      ...
      meta={<PeriodNote fallback="current calendar month" />}
      actions={
        <Link className="text-xs text-link hover:underline" to="/calendar">
          Calendar →
        </Link>
      }
    >
    ...
    >
      {hires.length === 0 ? (
        <EmptyState
          ...
          description="No recent hires are available."
        />
      ) : (
        <div className="space-y-1.5 text-sm">
          {hires.map((h) => (
    ...
    >
      {leaves.length === 0 ? (
        <EmptyState
          ...
          description="Every leave request has been actioned."
        />
      ) : (
        <div className="space-y-1.5 text-sm">
          {leaves.map((l) => (
