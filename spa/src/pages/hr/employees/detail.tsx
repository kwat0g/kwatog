...
const TABS = ['Overview', 'Employment history', 'Attendance', 'Leaves', 'Loans', 'Documents', 'Property', 'Trainings', 'Skills', 'Payroll', 'Activity'] as const;
...
export default function EmployeeDetailPage() {
 ...
 {tab === 'Property' && <PropertyTab employee={{ id } as any} />}
          ...
          <div className="shrink-0">
            ...
            {can('hr.employees.edit') && (
              <div className="mt-1 flex gap-1">
                ...
                {employee.photo_url && (
                  <button className="text-2xs text-danger hover:underline"
                    onClick={() => deletePhotoMutation.mutate()}>Remove</button>
                )}
              </div>
            )}
          </div>
...
function TrainingsTab({ employeeId }: { employeeId: string }) {
 ...
 mutationFn: (recordId: string) => employeeTrainingsApi.complete(recordId, { completed_at: new Date().toISOString().slice(0, 10) }),
 onSuccess: () => { qc.invalidateQueries({ queryKey: ['employee-trainings', employeeId] }); toast.success('Training completed.'); },
 onError: () => toast.error('Failed to complete training.'),
 });

 const cancelMutation = useMutation({
 mutationFn: (recordId: string) => employeeTrainingsApi.cancel(recordId),
 onSuccess: () => { qc.invalidateQueries({ queryKey: ['employee-trainings', employeeId] }); toast.success('Training cancelled.'); },
 ...
 return (
 <>
 <Panel title={`Trainings (${rows.length})`} noPadding
 actions={can('hr.employees.trainings.manage') ? (
 <Button variant="secondary" size="sm" icon={<Plus size={12} />} onClick={() => setShowAssign(true)}>Assign</Button>
 ) : null}
 >
 {rows.length === 0 ? <div className="p-4"><EmptyState icon="file-text" title="No trainings assigned" /></div> : (
 <table className={tableCls}>
 <thead><tr className={theadTrCls}><Th>Training</Th><Th>Scheduled</Th><Th>Completed</Th><Th>Expires</Th><Th>Status</Th><Th /></tr></thead>
 <tbody>
 {rows.map((r: any) => (
 <tr key={r.id} className={trCls}>
 ...
 <Td><Chip variant={chipVariantForStatus(r.status)}>{r.status_label}</Chip></Td>
 <Td>
 {r.status === 'scheduled' && can('hr.employees.trainings.manage') && (
 <div className="flex gap-1">
 <Button variant="ghost" size="sm" icon={<Check size={12} />} onClick={() => completeMutation.mutate(r.id)} />
 ...
 {showAssign && <AssignTrainingModal employeeId={employeeId} onClose={() => setShowAssign(false)} />}
 </>
 );
}
...
function AssignTrainingModal({ employeeId, onClose }: { employeeId: string; onClose: () => void }) {
 const qc = useQueryClient();
 const { data: trainingsResp } = useQuery({ queryKey: ['hr', 'trainings', 'all'], queryFn: () => trainingsApi.list({ per_page: 200 }) });
 const trainings = trainingsResp?.data ?? [];
 const { register, handleSubmit, formState: { errors } } = useForm<{ training_id: string; scheduled_for: string }>({
 defaultValues: { scheduled_for: new Date().toISOString().slice(0, 10) },
 });
 const mutation = useMutation({
 mutationFn: (d: { training_id: string; scheduled_for: string }) => employeeTrainingsApi.assign(employeeId, d),
 onSuccess: () => { qc.invalidateQueries({ queryKey: ['employee-trainings', employeeId] }); toast.success('Training assigned.'); onClose(); },
 ...
 <select {...register('training_id', { required: 'Required' })} className="w-full h-9 px-3 rounded-md border border-default bg-canvas text-sm">
 <option value="">— Select —</option>
 {trainings.filter((t) => t.is_active).map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
 </select>
 {errors.training_id && <p className="text-xs text-danger mt-1">{errors.training_id.message}</p>}
 </div>
 ...
 <Button type="submit" variant="primary" disabled={mutation.isPending} loading={mutation.isPending}>Assign</Button>
 </div>
 </form>
 </Modal>
 );
...
function SkillsTab({ employeeId }: { employeeId: string }) {
 ...
 onSuccess: () => { qc.invalidateQueries({ queryKey: ['employee-skills', employeeId] }); toast.success('Skill removed.'); },
 ...
 return (
 <>
 <Panel title={`Skills (${rows.length})`} noPadding
 actions={can('hr.employees.trainings.manage') ? (
 <Button variant="secondary" size="sm" icon={<Plus size={12} />} onClick={() => setShowAssign(true)}>Assign</Button>
 ) : null}
 >
 {rows.length === 0 ? <div className="p-4"><EmptyState icon="file-text" title="No skills assigned" /></div> : (
 <table className={tableCls}>
 <thead><tr className={theadTrCls}><Th>Skill</Th><Th>Category</Th><Th>Level</Th><Th>Acquired</Th><Th>Expires</Th><Th /></tr></thead>
 <tbody>
 {rows.map((r: any) => (
 <tr key={r.id} className={trCls}>
 <Td>{r.skill?.name ?? '—'}</Td>
 ...
 <Button variant="ghost" size="sm" onClick={() => removeMutation.mutate(r.id)}>Remove</Button>
 )}
 </Td>
 </tr>
 ))}
 ...
 {showAssign && <AssignSkillModal employeeId={employeeId} onClose={() => setShowAssign(false)} />}
 </>
 );
}
...
function AssignSkillModal({ employeeId, onClose }: { employeeId: string; onClose: () => void }) {
 const qc = useQueryClient();
 const { data: skillsResp } = useQuery({ queryKey: ['hr', 'skills', 'all'], queryFn: () => skillsApi.list({ per_page: 200 }) });
 const skills = skillsResp?.data ?? [];
 const { register, handleSubmit, formState: { errors } } = useForm<{ skill_id: string; proficiency_level: string }>();
 const mutation = useMutation({
 mutationFn: (d: { skill_id: string; proficiency_level: string }) => employeeSkillsApi.assign(employeeId, d),
 onSuccess: () => { qc.invalidateQueries({ queryKey: ['employee-skills', employeeId] }); toast.success('Skill assigned.'); onClose(); },
 ...
 <select {...register('skill_id', { required: 'Required' })} className="w-full h-9 px-3 rounded-md border border-default bg-canvas text-sm">
 <option value="">— Select —</option>
 {skills.filter((s) => s.is_active).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
 </select>
 {errors.skill_id && <p className="text-xs text-danger mt-1">{errors.skill_id.message}</p>}
 </div>
 <div>
 <label className="text-xs text-muted font-medium mb-1 block">Proficiency</label>
 <select {...register('proficiency_level')} className="w-full h-9 px-3 rounded-md border border-default bg-canvas text-sm">
 <option value="">— Select —</option>
...
function PayrollTab({ employeeId }: { employeeId: string }) {
 ...
 return client.get('/payroll/payrolls', { params: { employee_id: employeeId, per_page: 25 } }).then((r) => r.data);
 },
 });
 const rows = data?.data ?? [];

 if (isLoading) return <SkeletonPanel />;
 if (rows.length === 0) return <Panel title="Payroll history"><EmptyState icon="file-text" title="No payroll records yet" /></Panel>;
 return (
 <Panel title={`Payroll history (${rows.length})`} noPadding>
 <table className={tableCls}>
 <thead><tr className={theadTrCls}><Th>Period</Th><Th>Cutoff</Th><Th align="right">Basic</Th><Th align="right">Gross</Th><Th align="right">Deductions</Th><Th align="right">Net</Th><Th>Status</Th></tr></thead>
 <tbody>
 {rows.map((r: any) => (
 <tr key={r.id} className={trCls}>
 <Td mono>{r.payroll_period?.label ?? r.payroll_period_id}</Td>
 <Td mono>{r.payroll_period?.period_start?.slice(0, 10)} – {r.payroll_period?.period_end?.slice(0, 10)}</Td>
 <Td align="right" mono>{formatPeso(r.basic_pay ?? 0)}</Td>
 <Td align="right" mono>{formatPeso(r.gross_pay ?? 0)}</Td>
 <Td align="right" mono>{formatPeso(r.total_deductions ?? 0)}</Td>
 <Td align="right" mono className="font-medium">{formatPeso(r.net_pay ?? 0)}</Td>
 <Td><Chip variant={chipVariantForStatus(r.payroll_period?.status ?? 'draft')}>{(r.payroll_period?.status_label ?? r.payroll_period?.status ?? 'draft')}</Chip></Td>
 </tr>
 ))}
 </tbody>
 </table>
