  ...
  const mutation = useMutation({
    ...
    onError: (err: AxiosError<ApiValidationError>) => {
      if (err.response?.status === 422 && err.response.data.errors) {
        ...
        toast.error(
          isEdit ? 'Failed to update succession plan.' : 'Failed to create succession plan.',
        );
      }
    },
  });
      ...
      >
        <fieldset className="mb-6">
          ...
          >
            ...
            {(planOptions?.positions ?? []).map((option) => (
          ...
          <div className="grid grid-cols-2 gap-3 mt-3">
            ...
            >
              ...
              {(planOptions?.employees ?? []).map((option) => (
                <option key={option.value} value={option.value}>
                  {option.label}
                </option>
              ))}
        ...
        <fieldset className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              {(planOptions?.readiness ?? []).map((option) => (
                <option key={option.value} value={option.value}>
                  {option.label}
                </option>
              ))}
            ...
            >
              <option value="">— Select —</option>
              {(planOptions?.priorities ?? []).map((option) => (
                <option key={option.value} value={option.value}>
                  {option.label}
                </option>
              ))}
        ...
        <div className="flex items-center justify-end gap-2 pt-4 border-t border-default">
          <Button
            type="button"
            variant="secondary"
            onClick={() => navigate('/hr/succession-plans')}
          >
            Cancel
          </Button>
          <Button
