...
export default function PostingEditPage() {
  ...
  const form = useForm<FormData>({
    ...
    values: posting
      ? {
          ...
          slots: posting.slots,
          closes_at: formatDateIso(posting.closes_at, ''),
        }
      : undefined,
  });
  const {
  ...
  const mutation = useMutation({
    mutationFn: (data: FormData) =>
      recruitmentApi.updatePosting(id!, data as Parameters<typeof recruitmentApi.updatePosting>[1]),
    onSuccess: () => {
  ...
  return (
    <div>
      ...
      <form
        onSubmit={handleSubmit((d) => mutation.mutate(d), onFormInvalid<FormData>())}
      ...
      >
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
              <Input
                ...
                step="0.01"
                prefix="₱"
                error={errors.salary_range_min?.message}
              />
              <Input
                label="Salary Max"
                ...
                step="0.01"
                prefix="₱"
                error={errors.salary_range_max?.message}
              />
              <Input
                label="Slots"
                {...register('slots')}
            ...
            <Checkbox
              id="show_salary"
              label="Show salary range on public listing"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Panel
            title={
              <>
                Description <span className="text-danger-fg ml-0.5">*</span>
              </>
            }
          >
            <Textarea
          ...
          <Panel
            title={
              <>
                Requirements <span className="text-danger-fg ml-0.5">*</span>
              </>
            }
          >
              ...
              {reqTags.length > 0 ? (
                  ...
                  {reqTags.map((tag, i) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className={cn(
                          'ml-2 text-muted hover:text-danger-fg transition-colors shrink-0 cursor-pointer rounded',
                          focusRing,
                        )}
                      >
                ...
                !errors.requirements && (
                  <p className="text-xs text-muted">
                    Press Enter or click Add after typing each requirement.
                  </p>
                )
              )}
            </div>
        ...
        <FormActions>
          <Button
            type="button"
            variant="secondary"
            onClick={() => navigate(`/hr/recruitment/postings/${id}`)}
          >
            Cancel
          </Button>
          <Button
