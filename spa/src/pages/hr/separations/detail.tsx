...
export default function SeparationDetailPage() {
  ...
  const statusLabel = separationOptions?.statuses?.find(
    (option) => option.value === data?.status,
  )?.label;
  const reasonLabel = separationOptions?.reasons?.find(
    (option) => option.value === data?.separation_reason,
  )?.label;
  ...
  const steps: ChainStep[] = [
    { key: 'initiated', label: 'Initiated', state: 'done' },
    {
      key: 'items_cleared',
      label: 'Items cleared',
      state: data.cleared_count === data.items_total ? 'done' : 'active',
    },
    {
      key: 'final_pay',
      label: 'Final pay computed',
      state: data.final_pay_computed
        ? 'done'
        : data.cleared_count === data.items_total
          ? 'active'
          : 'pending',
    },
    {
      key: 'finalized',
      label: 'Finalized',
      state: data.status === 'finalized' ? 'done' : data.final_pay_computed ? 'active' : 'pending',
    },
  ];
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <div className="flex gap-1.5 items-center">
            ...
            {data.final_pay_computed &&
              data.status !== 'finalized' &&
              can('hr.separation.finalize') && (
                <Button
                  variant="primary"
                  size="sm"
      ...
      <div className="px-5 pb-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-2">
        ...
        <StatCard
          label="Final pay"
          value={data.final_pay_amount ? formatPeso(data.final_pay_amount) : '—'}
        />
      </div>
              ...
              <ul className="divide-y divide-subtle">
                {items.map((item) => (
                  <li key={item.item_key} className="py-2 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted font-mono">
                        {item.status === 'cleared' && item.signed_at ? (
                          <>
                            <LuCheck size={11} className="inline mr-1" />
                            Cleared {formatDateIso(item.signed_at)}
                          </>
                        ) : (
                          'Pending'
                        )}
                    ...
                    {item.status === 'pending' &&
                      data.status !== 'finalized' &&
                      can('hr.clearance.sign') && (
                        <Button
                          variant="secondary"
                          size="sm"
                          onClick={() => sign.mutate(item.item_key)}
