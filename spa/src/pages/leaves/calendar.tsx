  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          data
            ? `${data.headcount} active employees${departmentId ? ' in department' : ''}`
            : undefined
        }
        backTo="/hr/leaves"
                      ...
                      <div className="max-w-[200px] text-left whitespace-normal">
                        {cell.employees_on_leave.map((e, j) => (
                          <div key={j} className="truncate">
                            ...
                            {e.half_day_period ? ` · ${e.half_day_period.toUpperCase()}` : ''}) -{' '}
                            {e.status_label ?? e.status.replace(/_/g, ' ')}
                          </div>
                        ))}
                      </div>
                    ) : (
                ...
                >
                  <button
                    ...
                    className={cn(
                      ...
                      focusRing,
                      cell.date === localIsoDate() ? 'ring-2 ring-accent' : '',
                      selectedDay?.date === cell.date ? 'ring-2 ring-accent' : '',
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      {(cell.approved_count > 0 || cell.pending_count > 0) && (
                        <span
                          className={cn(
                            'text-2xs font-mono tabular-nums font-medium',
                            coverageTextColor(cell.coverage_pct, successPct, warningPct),
                          )}
                        >
                          {cell.coverage_pct}%
                        </span>
