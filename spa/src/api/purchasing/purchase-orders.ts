...
export const purchaseOrdersApi = {
  options: () =>
    client
      ...
      }>('/purchasing/purchase-orders/options')
      .then((r) => r.data),
  list: (
    params?: ListParams & {
      status?: string;
    ...
    client
      .get<PaginatedResponse<PurchaseOrder>>('/purchasing/purchase-orders', { params, signal })
      .then((r) => r.data),
  show: (id: string) =>
  ...
  acknowledgeBudget: (id: string) =>
    client
      .patch<PurchaseOrder>(`/purchasing/purchase-orders/${id}/acknowledge-budget`)
      .then((r) => r.data),
  approve: (id: string, remarks?: string) =>
    client
      .patch<PurchaseOrder>(`/purchasing/purchase-orders/${id}/approve`, { remarks })
      .then((r) => r.data),
  reject: (id: string, reason: string) =>
    client
      .patch<PurchaseOrder>(`/purchasing/purchase-orders/${id}/reject`, { reason })
      .then((r) => r.data),
  send: (id: string) =>
    client.patch<PurchaseOrder>(`/purchasing/purchase-orders/${id}/send`).then((r) => r.data),
  cancel: (id: string, reason: string) =>
    client
      .patch<PurchaseOrder>(`/purchasing/purchase-orders/${id}/cancel`, { reason })
      .then((r) => r.data),
  close: (id: string) =>
    client.patch<PurchaseOrder>(`/purchasing/purchase-orders/${id}/close`).then((r) => r.data),
  shortClose: (id: string, reason: string) =>
    client
      .patch<PurchaseOrder>(`/purchasing/purchase-orders/${id}/short-close`, { reason })
      .then((r) => r.data),
  ...
  responses: (id: string) =>
    client
      .get<PurchaseOrderResponse[]>(`/purchasing/purchase-orders/${id}/responses`)
      .then((r) => r.data),
  /** Accept a supplier's response (records the confirmed date/quantities/prices). */
  acceptResponse: (responseId: string) =>
    client
      .patch<PurchaseOrderResponse>(`/purchasing/purchase-order-responses/${responseId}/accept`)
      .then((r) => r.data),
  /** Reject a supplier's response with a reason. */
  rejectResponse: (responseId: string, reason: string) =>
    client
      .patch<PurchaseOrderResponse>(`/purchasing/purchase-order-responses/${responseId}/reject`, {
        reason,
      })
      .then((r) => r.data),
};
...
export const procurementChainApi = {
  overview: () => client.get<ProcurementChainOverview>('/purchasing/chain').then((r) => r.data),
};

export const threeWayMatchApi = {
  forBill: (billId: string) =>
    client
      .get<ThreeWayMatchResult | null>(`/purchasing/three-way-match/${billId}`)
      .then((r) => r.data),
};
