...
export const supplierPortalApi = {
  ...
  login: async (email: string, password: string) => {
    await getPortalCsrf();
    const { data } = await portalClient.post<{ data: SupplierLoginResponse }>(
      '/b2b/supplier/login',
      { email, password },
    );
  ...
  logout: async () => {
    try {
      ...
      /* the server invalidates the HTTP-only session */
    }
  },
  ...
  forgotPassword: async (email: string) => {
    await getPortalCsrf();
    const { data } = await portalClient.post<{ message: string }>('/b2b/supplier/forgot-password', {
      email,
    });
    return data;
  },
  ...
  changePassword: async (payload: {
    ...
    const { data } = await portalClient.post<{ message: string }>(
      '/b2b/supplier/change-password',
      payload,
    );
    return data;
  },
  ...
  businessPolicies: async () => {
    ...
    const { data } = await portalClient.get<{ data: BusinessPolicies }>(
      '/b2b/supplier/business-policies',
    );
    return data.data;
  },
  ...
  // ── Dashboard ──────────────────────────────────────
  dashboard: async () => {
    const { data } = await portalClient.get<{ data: SupplierDashboardData }>(
  ...
  listPos: async (params?: {
    ...
    const { data } = await portalClient.get<PaginatedResponse<PortalPoSummary>>(
      '/b2b/supplier/purchase-orders',
      { params },
    );
    return data;
  },
  ...
  getPo: async (id: string) => {
    const { data } = await portalClient.get<{ data: PortalPoDetail }>(
      `/b2b/supplier/purchase-orders/${id}`,
    );
    return data.data;
  },
  ...
  acknowledgePo: async (id: string) => {
    const { data } = await portalClient.post<{ message: string }>(
      `/b2b/supplier/purchase-orders/${id}/acknowledge`,
    );
    return data;
  },
  ...
  respondToPurchaseOrder: async (id: string, payload: RespondToPurchaseOrderPayload) => {
    const { data } = await portalClient.post<{ data: PortalPoDetail }>(
      `/b2b/supplier/purchase-orders/${id}/respond`,
      payload,
    );
    return data.data;
  },
  ...
  createShipment: async (
    ...
    const { data } = await portalClient.post<{ data: PortalShipment; message: string }>(
      `/b2b/supplier/purchase-orders/${poId}/shipments`,
      form,
    );
    return data;
  },

  updateShipmentById: async (
    poId: string,
    shipmentId: string,
    form: {
      shipped_date?: string;
    ...
    const { data } = await portalClient.put<{ data: PortalShipment; message: string }>(
      `/b2b/supplier/purchase-orders/${poId}/shipments/${shipmentId}`,
      form,
    );
    return data;
  },
  ...
  listInvoices: async (params?: { status?: string; page?: number; per_page?: number }) => {
    const { data } = await portalClient.get<PaginatedResponse<SupplierBillSummary>>(
  ...
  getInvoice: async (id: string) => {
    const { data } = await portalClient.get<{ data: SupplierBillDetail }>(
      `/b2b/supplier/invoices/${id}`,
    );
    return data.data;
  },
  ...
  listDeliveries: async (params?: { status?: string; page?: number; per_page?: number }) => {
    const { data } = await portalClient.get<PaginatedResponse<SupplierDeliverySummary>>(
      '/b2b/supplier/deliveries',
      { params },
    );
    return data;
  },
  ...
  statementOfAccount: async () => {
    const { data } = await portalClient.get<{ data: VendorStatementOfAccount }>(
      '/b2b/supplier/statement-of-account',
    );
    return data.data;
  },

  // ── Delivery Schedules ──────────────────────────────
  listDeliverySchedules: async (params?: {
    page?: number;
    per_page?: number;
    status?: string;
    purchase_order_id?: string;
  }) => {
    const { data } = await portalClient.get<PaginatedResponse<DeliverySchedule>>(
      '/b2b/supplier/delivery-schedules',
      { params },
    );
    return data;
  },
  ...
  getEligiblePurchaseOrders: async () => {
    const { data } = await portalClient.get<{ data: SchedulablePoOption[] }>(
      '/b2b/supplier/delivery-schedules/purchase-orders',
    );
    return data.data;
  },
  ...
  createDeliverySchedule: async (form: {
    ...
    const { data } = await portalClient.post<{ data: DeliverySchedule; message: string }>(
      '/b2b/supplier/delivery-schedules',
      form,
    );
    return data;
  ...
  cancelDeliverySchedule: async (id: string, reason: string) => {
    const { data } = await portalClient.post<{ data: DeliverySchedule; message: string }>(
      `/b2b/supplier/delivery-schedules/${id}/cancel`,
      { reason },
    );
    return data;
  },
  ...
  shippingDocumentOptions: async () => {
    const { data } = await portalClient.get<{
      data: { document_types: Array<{ value: string; label: string }> };
    }>('/b2b/supplier/purchase-orders/shipping-documents/options');
    return data.data;
  },
  listShippingDocuments: async (poId: string) => {
    const { data } = await portalClient.get<{ data: PortalShippingDocument[] }>(
  ...
  itemCatalog: async (params?: {
    search?: string;
    item_type?: string;
    page?: number;
    per_page?: number;
  }) => {
    const { data } = await portalClient.get<PaginatedResponse<PortalItemCatalogEntry>>(
      '/b2b/supplier/item-catalog',
      { params },
    );
    return data;
  },
  ...
  listItemListings: async (params?: { page?: number; per_page?: number; status?: string }) => {
    const { data } = await portalClient.get<PaginatedResponse<PortalSupplierListing>>(
      '/b2b/supplier/item-listings',
      { params },
    );
    return data;
  },
  ...
  createItemListing: async (form: PortalSupplierListingInput) => {
    const { data } = await portalClient.post<{ data: PortalSupplierListing }>(
      '/b2b/supplier/item-listings',
      form,
    );
    return data.data;
  ...
  bulkCreateItemListings: async (items: PortalSupplierListingInput[]) => {
    const { data } = await portalClient.post<{ data: PortalBulkListingResult }>(
      '/b2b/supplier/item-listings/bulk',
      { items },
    );
    return data.data;
  },
  ...
  updateItemListing: async (id: string, form: PortalSupplierListingInput) => {
    const { data } = await portalClient.put<{ data: PortalSupplierListing }>(
      `/b2b/supplier/item-listings/${id}`,
      form,
    );
    return data.data;
  },
