...
export const stockAdjustmentsApi = {
 options: () => client.get<{ data: { statuses: Array<{ value: string; label: string }>; directions: Array<{ value: string; label: string }>; reasons: Array<{ value: string; label: string }> } }>('/inventory/stock-adjustments/options').then((r) => r.data.data),
 ...
 create: (data: { item_id: string; location_id: string; direction: 'in' | 'out'; quantity: string; unit_cost?: string; reason_code?: string; reason: string }) =>
 client.post<ApiSuccess<StockAdjustment>>('/inventory/stock-adjustments', data).then((r) => r.data.data),
 approve: (id: string) =>
