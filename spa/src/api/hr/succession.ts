import { client } from '../client';
import type { ApiSuccess, PaginatedResponse, ListParams } from '@/types';
import type {
  SuccessionPlan,
  SuccessionReadiness,
  SuccessionPriority,
  SuccessionStatus,
  CreateSuccessionPlanData,
} from '@/types/succession';

export interface SuccessionPlanListParams extends ListParams {
  readiness?: SuccessionReadiness;
  priority?: SuccessionPriority;
  status?: SuccessionStatus;
}

export interface SuccessionOption {
  value: string;
  label: string;
}

export interface SuccessionPlanOptions {
  statuses: SuccessionOption[];
  readiness: SuccessionOption[];
  priorities: SuccessionOption[];
  positions: SuccessionOption[];
  employees: SuccessionOption[];
}

export const successionPlansApi = {
  options: () =>
    client
      .get<{ data: SuccessionPlanOptions }>('/hr/succession-plans/options')
      .then((r) => r.data.data),
  list: (params?: SuccessionPlanListParams) =>
    client
      .get<PaginatedResponse<SuccessionPlan>>('/hr/succession-plans', { params })
      .then((r) => r.data),
  show: (id: string) =>
    client.get<ApiSuccess<SuccessionPlan>>(`/hr/succession-plans/${id}`).then((r) => r.data.data),
  create: (data: CreateSuccessionPlanData) =>
    client.post<ApiSuccess<SuccessionPlan>>('/hr/succession-plans', data).then((r) => r.data.data),
  update: (id: string, data: Partial<CreateSuccessionPlanData>) =>
    client
      .put<ApiSuccess<SuccessionPlan>>(`/hr/succession-plans/${id}`, data)
      .then((r) => r.data.data),
  destroy: (id: string) => client.delete(`/hr/succession-plans/${id}`),
  restore: (id: string) => client.patch(`/hr/succession-plans/${id}/restore`),
};
