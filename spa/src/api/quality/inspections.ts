...
export const inspectionsApi = {
    ...
    client
      .get<{
        data: {
          ...
          measurement_results: Array<{ value: string; label: string }>;
          sampling_methods: Array<{ stage: string; value: string; label: string }>;
        };
      }>('/quality/inspections/options')
      .then((r) => r.data.data),
  workOrderOutputs: (productId: string) =>
    client
      .get<{
        data: WorkOrderOutputOption[];
      }>('/quality/inspections/work-order-outputs', { params: { product_id: productId } })
      .then((r) => r.data.data),
  list: (params?: InspectionListParams) =>
    client
      .get<PaginatedResponse<Inspection>>('/quality/inspections', { params })
      .then((r) => r.data),
  show: (id: string) =>
  ...
  recordMeasurements: (id: string, data: RecordMeasurementsData) =>
    client
      ...
      .then((r) => r.data.data),
  recordLotResult: (id: string, data: RecordLotResultData) =>
    client
      .post<ApiSuccess<Inspection>>(`/quality/inspections/${id}/lot-result`, data)
      .then((r) => r.data.data),
  complete: (id: string) =>
    client
      .post<ApiSuccess<Inspection>>(`/quality/inspections/${id}/complete`)
      .then((r) => r.data.data),
  review: (id: string, decision: 'passed' | 'failed', remarks?: string) =>
    client
      .patch<ApiSuccess<Inspection>>(`/quality/inspections/${id}/review`, { decision, remarks })
      .then((r) => r.data.data),
  ...
  aqlPreview: (batchQuantity: number) =>
    client
      .get<{
        data: AqlPlan;
      }>('/quality/inspections/aql-preview', { params: { batch_quantity: batchQuantity } })
      .then((r) => r.data.data),
