...
export const ncrsApi = {
  ...
  show: (id: string) => client.get<ApiSuccess<Ncr>>(`/quality/ncrs/${id}`).then((r) => r.data.data),
  ...
  addAction: (
    ...
    client
      .post<ApiSuccess<NcrAction>>(`/quality/ncrs/${id}/actions`, data)
      .then((r) => r.data.data),
  due: (params?: { page?: number; per_page?: number }) =>
    client
      .get<PaginatedResponse<NcrAction>>('/quality/ncrs/effectiveness/due', { params })
      .then((r) => r.data),
  verifyAction: (
    ncrId: string,
    actionId: string,
    data: { effectiveness_status: EffectivenessStatus; notes: string },
  ) =>
    client
      .patch<ApiSuccess<NcrAction>>(`/quality/ncrs/${ncrId}/actions/${actionId}/verify`, data)
      .then((r) => r.data.data),
  setDisposition: (id: string, data: SetNcrDispositionData) =>
  ...
  bulkClose: (ncr_ids: string[], resolution_note?: string) =>
    client
      .post<{ data: BulkCloseResponse }>('/quality/ncrs/bulk-close', { ncr_ids, resolution_note })
      .then((r) => r.data.data),
};
