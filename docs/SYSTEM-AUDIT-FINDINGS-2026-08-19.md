...
### F-057 — Every PDF and every auth email throws on a by-the-book install, because the production env template omits sixteen keys the dev one has

- **Module / feature:** Deployment / Settings — `.env.production.example`, `SettingsSeeder`, migration `0123_seed_company_branding_settings`, `SettingsService::requiredString()`.
- **Related modules:** Accounting (`PdfService`), Purchasing (`PurchaseRequestPdfService`), Auth (welcome, password reset, password-reset link), CRM (8D complaint reports), Production (daily and weekly summaries), HR (interview email), Common (`BusinessPolicyController`).
- **Category:** Deployment correctness / silent skip.
- **Affected roles:** every user. A locked-out user cannot reset their password.
- **Current Behavior:** `company.legal_name`, `company.address`, `company.tin` and `company.vat_status` are read with `requiredString()`, which throws `Required setting X is missing or invalid` on an empty value. None of `COMPANY_LEGAL_NAME`, `COMPANY_ADDRESS`, `COMPANY_TIN`, `COMPANY_PHONE`, `COMPANY_EMAIL`, `COMPANY_VAT_STATUS`, `COMPANY_PUBLIC_URL`, `COMPANY_CERTIFICATION`, `COMPANY_LOGO_PATH`, `COMPANY_SALES_INBOX_EMAIL`, `COMPANY_EMPLOYEE_EMAIL_DOMAIN`, `COMPANY_LATITUDE`, `COMPANY_LONGITUDE`, `QUALITY_COC_MANAGER_NAME`, `QUALITY_COC_MANAGER_ROLE` or `PDF_FOOTER_DISCLAIMER` appeared in `.env.production.example` — all sixteen exist in `api/.env.example`, which DEPLOY.md never mentions.
- **Problem:** Measured on the live host after a clean install: 9 of 11 company settings were empty and `requiredString('company.legal_name')` threw. That breaks **every PDF**, **every auth email including password reset**, 8D reports, both production summaries, the interview email and `GET /api/v1/business-policy`. The two that worked — `company.sales_inbox_email` and `company.employee_email_domain` — worked only because migrations 0358 and 0308 insert a literal.
- **Real-world scenario:** A user forgets their password on a freshly deployed system. The reset email throws, so no mail is sent and nothing tells them why. There is no other recovery path.
- **Root Cause:** Two mechanisms that each look safe and are not, in combination. Migration 0123 inserts `env(KEY, '')`, so the settings row **exists and is empty** rather than absent — an absent row would at least read as unconfigured. `SettingsSeeder` lists these in `$deploymentKeys`, where a blank env value **skips the write** to avoid clobbering a configured row. Together: a blank env leaves an empty row that nothing reports, and the throw surfaces later, at the unrelated moment a PDF or email renders. Compounded by the same template drift as F-056 — two env examples with different key sets and only the incomplete one referenced by the runbook.
- **Recommended Improvement:** Declare all sixteen in `.env.production.example`, grouped by whether blank is actually safe, each stating what breaks. Three groups, because they are not equivalent: the required ones that throw; the ones a migration already backstops (sales inbox, employee domain, coordinates), where blank is genuinely fine; and `QUALITY_COC_MANAGER_*`, which should **stay** blank because a name there signs every Certificate of Conformance and must be a real consenting person — `CoCService` falls back to an em-dash and "Quality Management". A follow-up worth doing separately: make `SettingsSeeder` fail loudly under `APP_ENV=production` when a `requiredString` key would land empty, so the install refuses rather than deferring the throw.
- **Ideal Process:** An install either has the values its features require, or refuses to complete.
- **New Feature/Module Required:** No.
- **Cross-Module Impact:** Fixing the template also fixed a defect introduced by F-056's own remediation: `ADMIN_NAME=System Administrator` was written **unquoted**. Laravel's Dotenv tolerates it, but `scripts/deploy-update.sh:107` sources the file with `set -a; . ./.env` under `set -e`, so an unquoted space aborts the deploy at the pre-migration backup with `Bar: command not found`. Hit for real on the live host during this deploy. Both the template and the live `.env` are now quoted, and the quoting rule is stated where the trap first appears.
- **Evidence:** `api/app/Common/Services/SettingsService.php:41` (`requiredString`); `api/database/migrations/0123_seed_company_branding_settings.php:17-26` (`env(KEY, '')`); `api/database/seeders/SettingsSeeder.php:412-415` (`$deploymentKeys`); 19 `requiredString('company.*')` call sites across 11 files. Measured before: `company.legal_name` `*** EMPTY ***` and `production:send-daily-summary` reporting `Daily production summary email failed`. Measured after: `Daily production summary sent to 4 recipient(s)`. Regression proof: the F-057 acceptance gate asserts every `COMPANY_*`/`QUALITY_*`/`ADMIN_*`/`PDF_FOOTER_*` key in `api/.env.example` is also declared in `.env.production.example`, so the two templates cannot drift apart again.
- **Priority:** P1.
- **Impact:** On a by-the-book install, no PDF renders and no authentication email sends, including the only password-recovery path.
- **Complexity:** S.
- **Status note:** Registered and fixed 2026-08-20. Found by following F-055's own redeploy: the fix for the `filter_var` TypeError (PR #114) let `production:send-daily-summary` reach its notification, which then threw on `company.legal_name` — one silent failure hiding behind another.
