...
## Re-audit 2026-09-19 — remediated (FINISHED) 2026-09-23

Current verdict: **FINISHED**.

- Current fixes verified in this worktree: AQL unit counting, 8D failure signaling,
  declared sample enforcement, no-spec outgoing-QC fail-closed behavior,
  one return inspection per product, PPAP expiry/rejection controls, NCR rework linkage,
  supplier-return fail-closed behavior, spec-revision immutability, traceability
  gap signaling, 8D notification role validation, decimal string serialization in measurements,
  corrected stage enum docstrings, NCR tier-3 executive escalation to vice_president,
   separate crm.complaints.view read permission, calibration equipment evidence linkage, and
   high-risk inspection maker-checker review.
- Current residuals: stored-but-unused dead code and any future approval-policy expansion beyond
  incoming/outgoing release decisions.
...
supplier-return closure, revision immutability, traceability completeness, 8D notification role,
measurement decimal serialization, stage enum docstring, NCR tier-3 escalation role, complaint
read/manage permission separation, and calibration equipment evidence linkage findings from the
historical pass are now closed in the current worktree and covered by regression tests.
The remaining findings are:

1. **Dead code / stored-but-unused:**
...
2. **No approval workflow inside the inspection engine** — completion is single-actor and
   permission-gated (though PPAP has SoD, and the QMS as a whole is IATF-scoped).

### Follow-up fix (2026-09-23)

Full sampling is now bounded by `quality.full_sampling.max_units` (default 1,000), with
`0535_bound_full_quality_sampling.php` seeding the setting and `QualitySamplingBoundsTest` covering
the refusal before inspection rows are created. Larger batches must use a finite quality plan.

The lot-checklist completion path now preserves numeric measurement IDs while merging checklist and
piece results, and `Inspection` explicitly allows its enum-backed `inspection_mode` to be written.
`IncomingLotChecklistTest` covers the API route, checklist completion, NCR failure, and tolerance
failure paths.

High-risk incoming and outgoing inspections now stop at `awaiting_review`; a different user with
`quality.inspections.review` must approve the proposed result before GRN acceptance, delivery
release, NCR creation, or downstream QC events occur. In-process and return inspections remain
single-actor by policy.

Declared sample enforcement was implemented at `InspectionService::complete()` (an inspection
cannot finalise with fewer measured sample units than its declared `sample_size`), but the
no-plan incoming contract in `createIncomingForItem()` scaffolded exactly ONE verdict row while
declaring the full AQL sample size. Every single-screen `receiveWithQc` receipt therefore became
permanently uncompletable, and the two Inventory tests covering it were red.
`createIncomingForItem()` now scaffolds one `Overall incoming material verdict` row per declared
sample unit, so the declaration and the scaffold agree; the AQL accept/reject counts apply to
those units as designed. Verified by `tests/Feature/Inventory/GrnRejectionTest`,
`tests/Feature/Inventory/GrnGlPostingTest`, and the full Inventory (208) and Quality (154)
feature suites.
