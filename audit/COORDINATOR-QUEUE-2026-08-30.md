...
**COORDINATOR ERROR, 2026-08-31 — check your git range before declaring work lost.** After the second parent-process exit I ran `git log 33c43e8a..HEAD`, saw nothing, and told all three live agents (and the user) that **zero commits had landed**. Wrong: all three had committed a skeleton **and** findings *before* my own queue commit, so they were ancestors of `33c43e8a` and correctly excluded from that range. `git log --oneline -6 -- audit/domains/<module>/` is the check that actually answers the question. **The skeleton-commit rule DID take — every interrupted agent had its findings safely on disk.** Do not raise a data-loss alarm without a per-path history check.
...
- **ROOT CAUSE FOUND for the container cycling that poisoned runs all pipeline: a backgrounded `docker compose run` dying with its parent takes `db` and `redis` down with it (exit 255).** Measured by the `traceability-ppap` session, which then needed `docker compose start db redis` plus **72s** of `FATAL: the database system is starting up`. Not `--no-deps` alone — **never background a `docker compose run` or let one outlive the turn.** Every brief now says so. This is the missing half of the "0 assertions" mystery and belongs in CLAUDE.md.
- **`DocumentSequenceService` loses a first-of-month race, with the mechanism now pinned: a plain `insert()` where `lockOrCreate()` uses `insertOrIgnore()`.** Measured `SQLSTATE[23505]`; the deadlock retry does not cover it. **Affects every module's document numbering.** Shared `Common` scope, so no module session will own it. Separately `document_sequences` has **no `work_order` row**, so `WO-YYYYMM-NNNN` is predicted to 500 on first real use — check whether other types are missing rows too.
...
| 13 | 3 | M042 | inventory/material-issues-reservations | done → 📋 Plan Ready |
...
| 17 | 3 | M058 | quality/traceability-ppap | done |
| 18 | 3 | M051 | manufacturing/production-work-orders | done |
| 19 | 3 | M050 | manufacturing/capacity-scheduling | **in flight** |
| 20 | 3 | M053 | manufacturing/maintenance-machine-health | **in flight** |
| 21 | 3 | M048 | manufacturing/demand-forecasting | queued |
| 22 | 3 | M043 | supply-chain/import-shipments-customs | done |
| 23 | 3 | M044 | supply-chain/deliveries-proof | done → 📋 Plan Ready |
| 24 | 3 | M045 | supply-chain/fleet-driver | **in flight** (inherits M044's `fleet.manage` held-by-no-role finding) |
...
| M058 | quality/traceability-ppap | 🔁 Needs Re-audit | **0 of 12 endpoints had ever been hit over HTTP** and both new P0s sat in that gap. **The forward trace has never worked** — `whereJsonContains` encodes a JSON object against a column holding an array of objects, so `recall-simulation` answered `found:false / 0 customers` on a fully intact chain. And `POST shipment-lot` has **500'd for its entire life** (calls a method that does not exist), which is why `shipment_lots` has 0 rows. **9 of 11 trace links silently erase** on delete/archive. PPAP self-approval confirmed; no create-element route, so the 18 AIAG elements are unreachable. 5 fixed. |
| M042 | inventory/material-issues-reservations | 📋 Plan Ready | Six P0s, **zero fixed — and argued, not pleaded**: the spurious refusal is accidentally containing a second defect, so fixing the ordering alone would make currently-safe failures start succeeding *and* over-release other WOs' reservations. Two issues racing one reservation **both won** (10-unit reservation authorised 20). Issue accepted against all 7 WO states incl. `cancelled`/`closed`, and 8× over BOM. **Disproved the validation family for its own module**, as it predicted before probing. |
| M043 | supply-chain/import-shipments-customs | 🔁 Needs Re-audit | **15 of 21 endpoints had no HTTP test, and 3 of 4 P0/P1s lived on exactly those routes.** `calculate-landed-cost` **500'd for every shipment with ≥2 PO lines** — i.e. every real import — so `impex_officer` completed 14 of 15 documented steps and failed on step 5. `by_weight` allocation **had never once worked** and its basis column does not exist; made to refuse explicitly rather than silently equal-split. Apportionment does **not** sum to the total (7 lines over-allocate by ₱0.03). 7 fixed, 109→160 tests. |
