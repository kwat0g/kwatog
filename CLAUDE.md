...
- **Partial employment prorates BOTH ends.** Basic pay is flat per cutoff, which is only
  right for someone employed the whole cutoff. The shared calendar-day fraction
  `App\Modules\Payroll\Support\EmployedDayFraction::of(periodStart, periodEnd, hireDate,
  separationDate)` scales it by the days actually covered — hire date OR separation date
  (`clearances.separation_date`, earliest wins). It is the ONE implementation: payroll's
  calculator and HR's `FinalPayService` no-payroll-row fallback both delegate to it, never
  re-derive (HR-02). Without the separation half a leaver banked the full half-month and
...
- `App\Modules\Production\Models\WorkOrder::coversSalesOrderLine()` — the ONE rule for who replaces a batch that failed outgoing QC. True (root WO on an SO line) → MRP owns it: `QueueMrpOnOutgoingInspectionFailed` re-plans the order and NCR close creates no WO. False (stock batch) → NCR close creates the replacement/rework WO. Two owners doubled production (a 10-piece failed lot left 20 open).
- `App\Common\Events\ToleratesNewerModelState` — marks an outbox event as a FACT that may be delivered with a newer row than it was published with (the codec still refuses an older one). Only for events whose every listener re-reads current state and guards on it; everything else keeps the strict version pin. Without it `SalesOrderConfirmed` died whenever MRP touched the order first.
