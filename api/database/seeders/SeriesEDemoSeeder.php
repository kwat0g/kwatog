...
class SeriesEDemoSeeder extends Seeder
{
    public function run(
        ...
        $this->seedFallbackDemoDocuments($admin, $vault, $pdfRenderer);
        $this->seedScheduledExports($admin);
        $this->seedColumnPreferences($admin);
    }

    /**
     * Fallback: when there are no finalized payrolls or invoices yet, the
     * vault would be empty and the Documents tab would show "no documents
     * generated yet" forever. So if vault rows count is still 0 after the
     * payroll/invoice attempts, attach a few generic demo PDFs to existing
     * employees and customers using the new pdf.demo Blade.
     */
    private function seedFallbackDemoDocuments(User $admin, DocumentVaultService $vault, PdfRenderService $pdfRenderer): void
    {
        if (Document::query()->count() > 0) {
            return;
        }

        $employees = Employee::query()->orderBy('id')->limit(3)->get();
        $customers = Customer::query()->orderBy('id')->limit(2)->get();

        if ($employees->isEmpty() && $customers->isEmpty()) {
            $this->command?->info('SeriesEDemoSeeder: no employees or customers — vault stays empty. Run DemoDataSeeder first.');
            return;
        }
        ...
        foreach ($employees as $employee) {
            try {
                $bytes = $pdfRenderer->render(
                    'pdf.demo',
                    [
                        'subject'     => 'Welcome to Ogami ERP',
                        'reference'   => 'DEMO-EMP-'.$employee->employee_no,
                        'body'        => "Hi {$employee->full_name}, this sample document was generated by the Series E demo seeder. ".
                            'It is here so the document vault has visible content when no real payslips have been generated yet.',
                        'attached_to' => $employee->full_name.' ('.$employee->employee_no.')',
                    ],
                    [
                        'paper'        => 'a4',
                        ...
                        'confidential' => false,
                        'generator'    => $admin,
                        'title'        => 'Sample HR Document',
                    ],
                );
                $vault->store($bytes, DocumentType::WorkOrderTraveler, $employee, $admin, false);
                $count++;
            } catch (Throwable $e) {
                $this->command?->warn(
                    sprintf('SeriesEDemoSeeder: fallback PDF render failed for employee %d: %s', $employee->id, $e->getMessage()),
                );
            }
        }
        ...
        foreach ($customers as $customer) {
            try {
                $bytes = $pdfRenderer->render(
                    'pdf.demo',
                    [
                        'subject'     => 'Sample Customer Statement',
                        'reference'   => 'DEMO-CUST-'.$customer->id,
                        'body'        => "Sample of an outbound document for {$customer->name}. The Series E vault stores it the same way ".
                            'a real Tax Invoice or Statement of Account would be stored.',
                        'attached_to' => $customer->name,
                    ],
                    [
                        'paper'        => 'a4',
                        'orientation'  => 'portrait',
                        'confidential' => false,
                        'generator'    => $admin,
                        'title'        => 'Sample Customer Document',
                    ],
                );
                $vault->store($bytes, DocumentType::Coc, $customer, $admin, false);
                $count++;
            } catch (Throwable $e) {
                $this->command?->warn(
                    sprintf('SeriesEDemoSeeder: fallback PDF render failed for customer %d: %s', $customer->id, $e->getMessage()),
                );
            }
        }

        $this->command?->info("SeriesEDemoSeeder: seeded {$count} fallback demo vault rows.");
    }
