...
class DocumentAccessTest extends TestCase
{
    ...
    public function test_failed_shipment_document_insert_does_not_leave_an_orphaned_file(): void
    {
        $creator = $this->seedUserWithPerms([]);
        $shipment = $this->seedShipment($creator);
        $deletedUploader = $this->seedUserWithPerms([]);
        $deletedUploader->forceDelete();
        ...
        try {
            ...
            $this->fail('Expected the stale uploader foreign key to reject the document insert.');
        } catch (QueryException) {
            Storage::disk('local')->assertDirectoryEmpty("shipments/{$shipment->id}");
        }
    }

    public function test_rolled_back_document_delete_keeps_the_physical_file(): void
    {
        $user = $this->seedUserWithPerms([]);
        $shipment = $this->seedShipment($user);
        $path = "shipments/{$shipment->id}/rollback.pdf";
        Storage::disk('local')->put($path, 'pdf');
        $document = ShipmentDocument::create([
            'shipment_id' => $shipment->id,
            'document_type' => ShipmentDocumentType::CommercialInvoice->value,
            'file_path' => $path,
        ...
        DB::beginTransaction();
        try {
            app(ShipmentService::class)->deleteDocument($document);
        ...
        Storage::disk('local')->assertExists($path);
        $this->assertDatabaseHas('shipment_documents', ['id' => $document->id]);
    }
