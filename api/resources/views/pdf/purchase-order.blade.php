  ...
  <tr>
    <td>
      <div class="info-card">
        ...
        <div style="font-weight:bold; font-size:10pt; color:#0F172A; margin-bottom:2px;">{{ $po->vendor?->name ?? '—' }}</div>
    ...
    <td>
      <div class="info-card last">
        ...
        <div class="info-row"><span class="lbl">PO Number:</span> <span class="val mono" style="font-weight:bold;">{{ $po->po_number }}</span></div>
  ...
  <tbody>
    @foreach($po->items as $i => $line)
      <tr>
        ...
        <td class="mono" style="font-weight:bold;">{{ $line->item?->code ?? '—' }}</td>
        <td>
          <div style="font-weight:bold; color:#0F172A;">{{ $line->item?->name ?? $line->description }}</div>
          ...
          @endif
          @php $lineCharges = (float) ($line->rfq_line_freight_amount ?? 0) + (float) ($line->rfq_line_other_charges ?? 0); @endphp
          @if ($lineCharges > 0)
            <div style="font-size:7.5pt; color:#64748B;">Total includes line freight/charges ₱ {{ number_format($lineCharges, 2) }}</div>
          @endif
        </td>
        <td class="r mono">{{ number_format((float) $line->quantity, 2) }} {{ $line->unit }}</td>
        <td class="r mono">₱ {{ number_format((float) $line->unit_price, 2) }}</td>
        <td class="r mono" style="font-weight:bold;">₱ {{ number_format((float) $line->total, 2) }}</td>
      </tr>
    @endforeach
  </tbody>
</table>
...
<div class="totals-container">
  <table class="totals-box">
    @php $poHeaderCharges = (float) ($po->rfq_freight_amount ?? 0) + (float) ($po->rfq_other_charges ?? 0); @endphp
    @if($poHeaderCharges > 0)
      <tr>
        <td class="lbl">Lines:</td>
        <td class="val mono">₱ {{ number_format((float) $po->subtotal - $poHeaderCharges, 2) }}</td>
      </tr>
      <tr>
        <td class="lbl">Freight &amp; other charges:</td>
        <td class="val mono">₱ {{ number_format($poHeaderCharges, 2) }}</td>
      </tr>
    @endif
    <tr>
