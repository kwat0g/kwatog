...
@section('content')
  ...
  <table class="lines">
    <tbody>
      ...
      @endforeach
      <tr style="font-weight:bold;"><td>Total Assets</td><td class="r">{{ $currency }} {{ $money->format($data['assets']['total']) }}</td></tr>

      <tr><td colspan="2" style="font-weight:bold;background:#FAFAFA;">LIABILITIES</td></tr>
      @foreach ($data['liabilities']['accounts'] as $r)
        <tr><td>&nbsp;&nbsp;{{ $r['code'] }} — {{ $r['name'] }}</td><td class="r">{{ $currency }} {{ $money->format($r['amount']) }}</td></tr>
      @endforeach
      <tr style="font-weight:bold;"><td>Total Liabilities</td><td class="r">{{ $currency }} {{ $money->format($data['liabilities']['total']) }}</td></tr>

      <tr><td colspan="2" style="font-weight:bold;background:#FAFAFA;">EQUITY</td></tr>
      @foreach ($data['equity']['accounts'] as $r)
        <tr><td>&nbsp;&nbsp;{{ $r['code'] }} — {{ $r['name'] }}</td><td class="r">{{ $currency }} {{ $money->format($r['amount']) }}</td></tr>
      @endforeach
      <tr style="font-weight:bold;"><td>Total Equity</td><td class="r">{{ $currency }} {{ $money->format($data['equity']['total']) }}</td></tr>

      <tr style="font-weight:bold;border-top:2px solid #09090B;border-bottom:2px solid #09090B;"><td>Total Liabilities + Equity</td><td class="r">{{ $currency }} {{ $money->format($data['total_liabilities_equity']) }}</td></tr>
    </tbody>
  </table>
@endsection
