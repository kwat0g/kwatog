...
class RfqAwardService
{
     ...
     * @return array{rfq: RequestForQuote, purchase_orders: array<int, \App\Modules\Purchasing\Models\PurchaseOrder>}
     */
    public function award(RequestForQuote $rfq, array $rows, User $by): array
    {
        return DB::transaction(function () use ($rfq, $rows, $by): array {
            ...
            if (! $by->hasPermission('purchasing.rfq.award')) throw new BusinessRuleException('RFQ award permission is required.');
            if (! in_array($locked->status, [RfqStatus::Closed, RfqStatus::UnderEvaluation, RfqStatus::Awarded, RfqStatus::PartiallyAwarded], true)) throw new BusinessRuleException('Only closed RFQs can be awarded.');
            ...
            foreach ($rows as $row) {
                ...
                if ((int) $quote->request_for_quote_id !== $locked->id || $quote->status !== SupplierQuoteStatus::Submitted || ! $quote->is_current) throw new BusinessRuleException('Only the current submitted quotation for this RFQ can be awarded.');
                if ($quoteItem->response_status !== SupplierQuoteResponseStatus::Quoted) throw new BusinessRuleException('A no-quote line cannot be awarded.');
                if ($quoteItem->compliance_status === RfqComplianceStatus::Blocking) throw new BusinessRuleException('A blocking quality exception prevents this line from being awarded.');
                $rfqItem = $locked->items->firstWhere('id', (int) $row['request_for_quote_item_id']);
                if (! $rfqItem || (int) $quoteItem->request_for_quote_item_id !== (int) $rfqItem->id) throw new BusinessRuleException('The selected quotation line does not belong to this RFQ line.');
                $qty = (string) $row['awarded_quantity'];
                if (Money::lte($qty, '0') || Money::gt($qty, (string) $quoteItem->offered_quantity)) throw new BusinessRuleException('Awarded quantity must be positive and no more than the supplier offer.');
                if (Money::gt($qty, (string) $rfqItem->quantity)) throw new BusinessRuleException('Awarded quantity cannot exceed the RFQ quantity.');
            ...
            foreach ($locked->items as $line) {
                ...
                if (count($selected) === 1) {
                    ...
                    if ($responses === 1 && trim((string) ($selected[0][0]['single_response_justification'] ?? '')) === '') throw new BusinessRuleException('A single-response award requires a justification.');
                }
                $total = Money::zero();
                foreach ($selected as [$row, $quoteItem, $quote, $qty]) {
                    ...
                    $locked->awards()->create([
                        ...
                        'awarded_total_delivered_cost' => Money::mul($qty, (string) $quoteItem->unit_price),
                ...
                if (Money::gt($total, (string) $line->quantity)) throw new BusinessRuleException('Total awards for an RFQ line cannot exceed its requested quantity.');
            }
            ...
            $sourcePr = $locked->purchaseRequest->loadMissing('items');
            ...
            foreach ($awards->groupBy('vendor_id') as $vendorAwards) {
                ...
                $po = $this->purchaseOrders->create([
                    ...
                    'expected_delivery_date' => $first->rfqItem->required_delivery_date?->toDateString(),
                ...
                foreach ($po->items as $poItem) {
                    $award = $vendorAwards->firstWhere('purchase_request_item_id', $poItem->purchase_request_item_id);
                    if ($award) $poItem->forceFill(['rfq_award_id' => $award->id, 'supplier_quote_version_id' => $award->supplier_quote_id])->save();
                }
                $created[] = $po;
            }
            ...
            $locked->purchaseRequest()->update([
                'po_conversion_status' => 'converted',
                'po_conversion_note' => "Sourced through {$locked->rfq_number}.",
    ...
    {
        if ((int) $quoteItem->quote()->value('request_for_quote_id') !== (int) $rfq->id) {
            throw new BusinessRuleException('The quotation line does not belong to this RFQ.');
        }
        if (! $by->hasPermission('purchasing.rfq.quality_review')) throw new BusinessRuleException('RFQ quality-review permission is required.');
        if (! in_array($rfq->status, [RfqStatus::Closed, RfqStatus::UnderEvaluation], true)) throw new BusinessRuleException('Quality review is available after RFQ closure.');
        $quoteItem->forceFill([
            'compliance_status' => $data['compliance_status'],
            'compliance_notes' => $data['compliance_notes'] ?? null,
        ])->save();
        return $quoteItem->fresh(['quote', 'rfqItem']);
    }
}
