...
class RequestForQuoteController
{
    ...
    {
        try { $rfq = $this->rfqs->createFromPurchaseRequest($purchaseRequest, $request->validated(), $request->user()); }
    ...
    {
        try { return new RequestForQuoteResource($this->rfqs->show($rfq, $request->user())); }
        catch (BusinessRuleException $e) { abort(403, $e->getMessage()); }
    }

    public function update(UpdateRequestForQuoteRequest $request, RequestForQuote $rfq): RequestForQuoteResource
    {
        try { return new RequestForQuoteResource($this->rfqs->update($rfq, $request->validated(), $request->user())); }
        catch (BusinessRuleException $e) { abort(422, $e->getMessage()); }
    }

    public function publish(Request $request, RequestForQuote $rfq): RequestForQuoteResource
    {
        try { return new RequestForQuoteResource($this->rfqs->publish($rfq, $request->user())); }
        catch (BusinessRuleException $e) { abort(422, $e->getMessage()); }
    }

    public function extend(ExtendRequestForQuoteRequest $request, RequestForQuote $rfq): RequestForQuoteResource
    {
        try { return new RequestForQuoteResource($this->rfqs->extend($rfq, $request->validated(), $request->user())); }
        catch (BusinessRuleException $e) { abort(422, $e->getMessage()); }
    }

    public function addendum(StoreRfqAddendumRequest $request, RequestForQuote $rfq): RequestForQuoteResource
    {
        try { return new RequestForQuoteResource($this->rfqs->addendum($rfq, $request->validated(), $request->user())); }
        catch (BusinessRuleException $e) { abort(422, $e->getMessage()); }
    }

    public function comparison(Request $request, RequestForQuote $rfq): RequestForQuoteResource
    {
        try { return new RequestForQuoteResource($this->rfqs->comparison($rfq, $request->user())); }
        catch (BusinessRuleException $e) { abort(422, $e->getMessage()); }
    }

    public function award(AwardRfqRequest $request, RequestForQuote $rfq): JsonResponse
    {
        try { $result = $this->awards->award($rfq, $request->validated()['awards'] ?? [], $request->user()); }
        catch (BusinessRuleException $e) { abort(422, $e->getMessage()); }
    ...
    {
        try { return new SupplierQuoteItemResource($this->awards->reviewQuality($rfq, $quoteItem, $request->validated(), $request->user())); }
        catch (BusinessRuleException $e) { abort(422, $e->getMessage()); }
    }

    public function cancel(CancelRequestForQuoteRequest $request, RequestForQuote $rfq): RequestForQuoteResource
    {
        try { return new RequestForQuoteResource($this->rfqs->cancel($rfq, (string) $request->input('reason'), $request->user())); }
        catch (BusinessRuleException $e) { abort(422, $e->getMessage()); }
    }

    public function purchaseOrders(Request $request, RequestForQuote $rfq): AnonymousResourceCollection
    {
        try { $rfq = $this->rfqs->show($rfq, $request->user()); }
        catch (BusinessRuleException $e) { abort(403, $e->getMessage()); }
    ...
    public function downloadDocument(Request $request, RfqDocument $document)
    {
        abort_unless(
            $request->user()?->hasPermission('purchasing.rfq.evaluate')
                || $request->user()?->hasPermission('purchasing.rfq.quality_review'),
            403,
        );
