...
/**
 * A single customer response row. The `latest_response` block embedded in the
 * sales-order resources is a strict subset of this shape (no nested sales_order
 * / resolver) — keep the two in sync when changing the contract.
 */
class SalesOrderResponseResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            'id'                     => $this->hash_id,
            'sales_order_id'         => $this->whenLoaded('salesOrder', fn () => $this->salesOrder->hash_id),
